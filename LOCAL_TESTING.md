...
- The shared preload pins `MULTI_RUNTIME_ENABLED=false`, regardless of deployment settings inherited by the test runner. Multi-runtime tests explicitly set it to `"true"` for their cases and restore the prior value afterward; deleting it also returns to the product's default disabled behavior.
