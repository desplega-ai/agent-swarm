...
4. **Slack thread update** — if the completed/failed task has Slack metadata (slackChannelId/slackThreadTs), you MUST use \`slack-reply\` with the completed worker task's ID to post the result summary back to the original Slack thread. The person who asked in Slack is waiting for an answer there.
...
- **Started** (optional, for longer tasks): Brief note when you begin work (e.g. "Working on this now — implementing X")
