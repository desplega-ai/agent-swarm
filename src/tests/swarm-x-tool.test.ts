...
describe("swarm_x MCP tool", () => {
  test("routes composio requests with server-side auth and scrubbed output", async () => {
    process.env.COMPOSIO_API_KEY = "example-ck_tool_secret_value";
    const fetchMock = mock(async (url: string | URL | Request, init?: RequestInit) => {
      ...
      expect((init?.headers as Record<string, string>)["x-api-key"]).toBe(
        "example-ck_tool_secret_value",
      );
      return jsonResponse({ ok: true, token: "example-ck_tool_secret_value" });
    });
    ...
    expect(result.structuredContent.responseText).not.toContain("example-ck_tool_secret_value");
  });
