...
3. **Run seeded heartbeat data-gathering.** Use \`script-run\` with global script \`Heartbeat Audit\` and pass the current HEARTBEAT.md text as \`heartbeatMarkdown\`. It covers Rules #10/#13/#15/#16/#17: resolved stale PRs, pool-target risk schedules, schedule/provider failure clusters, and whether today's \`dream\` workflow run completed (the Dreaming add-on — fka compounding — absorbed the old daily-blocker-digest sweep). The Slack thread-reply check (Rule #11) stays Lead-side; the script runtime has no Slack token.
