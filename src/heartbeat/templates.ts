...
3. **Run seeded heartbeat data-gathering.** Use \`script-run\` with global script \`Heartbeat Audit\` and pass the current HEARTBEAT.md text as \`heartbeatMarkdown\`. It covers Rules #10/#13/#15/#16: resolved stale PRs, pool-target risk schedules, and schedule/provider failure clusters. Separately handle Rule #17 (the Dreaming add-on, fka compounding, absorbed the old daily-blocker-digest sweep): first check whether the \`dream-daily\` schedule is enabled — it ships disabled by default, and a disabled schedule means NO run is expected, so report nothing. Only when it is enabled, verify that today's \`dream\` workflow run completed via \`get-workflows\`/dashboard, and if your \`Heartbeat Audit\` script predates Dreaming, update it to include this conditional check. The Slack thread-reply check (Rule #11) stays Lead-side; the script runtime has no Slack token.
