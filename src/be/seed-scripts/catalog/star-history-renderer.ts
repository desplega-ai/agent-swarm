...
function niceStep(maximum: number, targetTickCount = 6) {
...
export function renderChart(repository: string, timestamps: number[], themeName: keyof typeof themes) {
  ...
  const x = (timestamp: number) => margin.left + ((timestamp - firstStar) / timeRange) * plotWidth;
  const y = (stars: number) => margin.top + plotHeight - (stars / yMaximum) * plotHeight;
