...
export function recordSwarmBusEvent(name: string, data: unknown): SwarmBusEvent {
  // Durable journal: webhook bodies and tool payloads can embed tokens, so the
  // persisted (and later re-read/delivered) form is scrubbed at this egress
  // point.
  const serialized = data === undefined ? null : scrubSecrets(JSON.stringify(data));
  const event: SwarmBusEvent = {
    id: crypto.randomUUID(),
    name,
    data: serialized === null ? undefined : JSON.parse(serialized),
    emittedAt: new Date().toISOString(),
  };
  getDb()
    .prepare("INSERT INTO swarm_events (id, name, data, emittedAt) VALUES (?, ?, ?, ?)")
    .run(event.id, event.name, serialized, event.emittedAt);
  return event;
}
...
/**
 * Recover deliveries orphaned mid-run by a crash/restart: rows stuck at
 * 'running' go back to 'pending' (attempts already counted at claim time) or
 * land on 'failed' when the attempt budget is spent. Single-process dispatcher,
 * so at boot every 'running' row is by definition orphaned.
 */
export function requeueOrphanedRunningDeliveries(maxAttempts: number): number {
  const now = new Date().toISOString();
  const db = getDb();
  const failed =
    db
      .prepare(
        `UPDATE subscription_deliveries
         SET status = 'failed', finishedAt = ?, error = 'orphaned at restart (attempt budget spent)'
         WHERE status = 'running' AND attempts >= ?`,
      )
      .run(now, maxAttempts).changes ?? 0;
  const requeued =
    db
      .prepare(
        `UPDATE subscription_deliveries
         SET status = 'pending', claimedAt = NULL, error = 'orphaned at restart, requeued'
