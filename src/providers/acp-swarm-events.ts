...
export function translateAcpSessionUpdate(update: SessionUpdate): ProviderEvent[] {
  switch (update.sessionUpdate) {
    ...
    // ACP adds session-update variants over time (1.4.0 introduced
    // compaction_update, compaction_summary_chunk, plan_update, plan_removed).
    // A generic provider must not drop a turn on an unknown kind, so anything
    // unrecognized passes through verbatim as a custom event.
    default:
      return [
        {
          type: "custom",
          name: `acp_${(update as { sessionUpdate: string }).sessionUpdate}`,
          data: update,
        },
      ];
  }
