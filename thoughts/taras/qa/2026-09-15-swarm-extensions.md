...
### Screenshots
- Extensions list with the agent-authored extension: `agent-fs://qa/agent-swarm/2026-09-15-extensions-qa/shot-ext-list.png`
- Extension detail after enabling from the dashboard: `agent-fs://qa/agent-swarm/2026-09-15-extensions-qa/shot-ext-detail-enabled.png`
- Run log with block and continue rows: `agent-fs://qa/agent-swarm/2026-09-15-extensions-qa/shot-ext-runs.png`
- Versions grid after rolling back to v1: `agent-fs://qa/agent-swarm/2026-09-15-extensions-qa/shot-ext-versions.png`
- List with four enabled extensions and the auto-disabled fixture: `agent-fs://qa/agent-swarm/2026-09-15-extensions-qa/shot-ext-list-final.png`
- Run log after fix 7, with Agent and Subject columns: `agent-fs://qa/agent-swarm/2026-09-15-extensions-qa/shot-ext-runs-agent.png`
- Worker's approval request answered by the operator: `agent-fs://qa/agent-swarm/2026-09-15-extensions-qa/shot-approval.png`
- Slack mock #general with blocked and routed tasks: `agent-fs://qa/agent-swarm/2026-09-15-extensions-qa/shot-slack-general.png`
- Slack mock #swarm-done with notifications posted by ext:notify-on-complete: `agent-fs://qa/agent-swarm/2026-09-15-extensions-qa/shot-slack-swarm-done.png`

Screenshots live in agent-fs under `qa/agent-swarm/2026-09-15-extensions-qa/`. Get a fresh link with `agent-fs signed-url <path> --json`. Signed links are posted in a PR comment, never committed.
...
- [x] 2. A Slack-origin block reported "Could not assign to: lead — error" and swallowed the reason. With `SLACK_RENDER_V2` on, a blocked mention got no reply at all (found by the new e2e assertion in CI, where earlier scenarios enable render v2). severity: major. Fixed in `src/slack/handlers.ts`: the catch surfaces `TaskCreationBlockedError.reason`, and the v2 branch posts the failed-assignment lines in the thread.
