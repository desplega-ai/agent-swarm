...
All gates green on the tip after step-9 (see step-9.md notes for the live e2e, unit suite, docs-site build, and Manual E2E outputs). Fresh-DB boot: the Manual E2E scratch DB applied all migrations and listed zero extensions. Existing-DB boot: a copy of a 145-tail DB applied only `155_extensions` (renumbered from 153 when merging main, which had taken 153 and 154). Docker: `bun run docker:build:api` needed one retry (registry tarball integrity errors on the first attempt); the container ran the Manual E2E flow end to end, proving `import()` of the staged hooks file inside the compiled binary.
