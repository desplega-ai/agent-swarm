...
- [ ] Root suites still green: `bun run test:root -- src/tests/apps-spike4.test.ts` (server untouched — sanity)

#### Automated QA (agent-browser against http://localhost:5375):
- [ ] Parity walkthrough on PM Inbox (`6f93f0ce`) + spike4_scratch (`11d1fef6`): main page renders, search/filter inputs work, page nav updates breadcrumbs + `/route` params, Drawer opens from row click, form create round-trips, zero console errors
- [ ] Via console: `window.__swarmAppsStore` shows `/apps/6f93f0ce…/queries/*` populated and NO unprefixed top-level `/queries` key
- [ ] Navigate PM Inbox → Notes Mini → back: PM Inbox query data is still present in the global store before the first poll returns (cross-app-warm)
- [ ] Hard-reload a deep link whose route param opens a Drawer (PM Inbox detail URL) → Drawer open on **first paint**, not a frame later (route-seed regression, review I3)
- [ ] `/dev/embed-test` renders a working AppSurface outside the apps route (embeddability directive)

#### Manual Verification:
- [ ] Taras: 5-minute click-around of 2–3 live apps on :5375 (SPA feel, no regressions — per standing manual-QA preference)
...
- [ ] Assembler unit tests via the **root runner** (`apps/ui` has no test runner — review I14): `bun run test:root -- src/tests/apps-element-assembly.test.ts` (test file imports `apps/ui/src/lib/json-render/assemble.ts` by relative path; the module is pure/alias-free by construction) — expansion, id/interaction rewrite across all `ELEMENT_KEYS` fields, prop substitution, `ElementSlot` splice, `/refs` data-ref rewrite, depth cap, unresolvable-target error node

#### Automated QA (agent-browser against :5375, using the Phase-4 fixtures incl. the "Element Consumer" app):
- [ ] Notes Mini's exported pure element rendered via `ElementRef` from Element Consumer: renders with consumer props; two instances of it on one page hold independent interaction state (type into instance 1's input → instance 2 unchanged — the collision fix, verified in-browser)
- [ ] Bound element from Notes Mini embedded in Element Consumer: shows Notes Mini's live query data; a row created in Notes Mini directly appears in the embed after poll; invoking the bound action from the embed writes to Notes Mini (verify via `GET /api/apps/bae5343b…/models/<m>/rows`) and the action slot lands under `/apps/<consumerId>/refs/bae5343b…/actions/*` in `window.__swarmAppsStore`
- [ ] Same-app reuse: an element used on two pages of one app works on both
- [ ] Break the float (on a **DB copy** — review I5: unexporting a referenced element is exactly what the compat gate rejects): apply the unexport with `forceElementBreak: ["<name>"]`, or mutate the copy's stored JSON directly via sqlite → consumer renders the inline error card, no white-screen; restore the live DB after

#### Manual Verification:
- [ ] Taras: judge the embed UX (loading states, error card) on :5375
...
#### Automated Verification:
- [ ] `bun run test:root -- src/tests/apps-rbac.test.ts`
- [ ] `bun run check:rbac-coverage` (new verb has live call sites; non-GET routes declared)
- [ ] `bun run lint && bun run tsc:check`
- [ ] `bun run docs:openapi` + commit
- [ ] Prior suites green: `bun run test:root -- src/tests/apps-spike5.test.ts src/tests/apps-elements.test.ts src/tests/apps-spike4.test.ts`

#### Automated QA (against :3113):
- [ ] Mint an `aswt_` token for a test user (`POST /api/users/{id}/mcp-tokens`); create a row through a named app route with that bearer → row's `createdBy` = `user:<id>` (sqlite/KV check)
- [ ] Same request with the operator key → `createdBy` = `operator` (favorites-precedent degradation)
- [ ] All 8 live apps still fully readable/usable with the operator key (anyAuthenticated parity — no behavior change for the dashboard)

#### Manual Verification:
- [ ] Review the verb descriptions in `permissions.ts` read as policy documentation
...
#### Automated Verification:
- [ ] `bun run test:root -- src/tests/apps-user-config.test.ts`
- [ ] All apps suites: `bun run test:root -- src/tests/apps-spike5.test.ts src/tests/apps-elements.test.ts src/tests/apps-rbac.test.ts src/tests/apps-spike.test.ts src/tests/apps-spike2.test.ts src/tests/apps-spike4.test.ts`
- [ ] `bun run lint && bun run tsc:check && bash scripts/check-db-boundary.sh && bun run check:rbac-coverage`
- [ ] `bun run docs:openapi` + commit; `bun run check:skill-sources`
- [ ] `cd apps/ui && bun run lint && bunx tsc -b`

#### Automated QA:
- [ ] Add a `userConfig` schema (e.g. `density: enum[compact,comfortable] default comfortable`) to PM Inbox via `app-patch`; agent-browser on :5375: gear opens the settings drawer, change density, save; `window.__swarmAppsStore` shows `/apps/6f93f0ce…/user/density` updated; reload persists it
- [ ] Bind a fixture element's prop to `{ "$state": "/user/density" }` → renders the stored value
- [ ] `app-rollback` PM Inbox across the schema-adding version → schema gone from definition, `sqlite3 /tmp/apps-spike-e2e.sqlite "SELECT * FROM app_user_config"` rows untouched; roll forward again → values resurface in the drawer

#### Manual Verification:
- [ ] Taras: settings drawer UX pass on :5375
