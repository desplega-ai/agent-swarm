...
| `--color-status-error` | `oklch(0.577 0.245 27.325)` (red-600) | `oklch(0.704 0.191 22.216)` (red-400) | `components/shared/status-badge.tsx:62` (`bg-red-500` + `text-red-600 dark:text-red-400`, FAILED); `:76` (UNHEALTHY); `:97` (REJECTED). Light value = red-600 for contrast against `--color-card`; dark = red-400 to match existing `dark:text-red-400` |
