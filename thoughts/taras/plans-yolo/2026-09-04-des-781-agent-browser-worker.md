...
- Pin `agent-browser` at 0.36.0 (npm latest). First pinned 0.31.1 to match Taras's Mac; Taras asked why not latest, there was no technical reason, so the bump was built and smoke-tested (build assert, `skills get core`, `snapshot -i`, screenshot, `--args` and `--executable-path` still present). (asked)
