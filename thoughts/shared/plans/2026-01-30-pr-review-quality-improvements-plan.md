...
## User Interaction Patterns for Memory Triggers

This section documents how PR creators, reviewers, and external agents can interact with the swarm's memory layer during PR reviews. These natural language triggers allow users to teach the swarm about their preferences, conventions, and project-specific patterns.

### Why Memory Triggers Matter

The swarm learns from your feedback to provide better reviews over time. Instead of seeing the same irrelevant comments repeatedly, you can teach the swarm what matters for your project.

### Memory Trigger Phrases

#### 1. "Remember this..." - Store Preferences

Use this phrase to teach the swarm about your conventions and preferences.

| Trigger Phrase | Effect | Scope |
|----------------|--------|-------|
| `remember this: ...` | Stores as semantic memory | Per-repo |
| `remember that ...` | Stores as semantic memory | Per-repo |
| `always remember: ...` | Stores as high-importance memory | Per-repo |
| `for future reviews: ...` | Stores as procedural memory | Per-repo |
| `our convention is: ...` | Stores as repo convention | Per-repo |

**Examples:**

```markdown
# In response to a style comment:
@desplega-bot remember this: we use 2-space indentation in all TypeScript files

# In response to a naming comment:
@desplega-bot remember that we prefix all React hooks with "use" even custom ones

# In response to a test coverage comment:
@desplega-bot for future reviews: files in /scripts/ don't need unit tests

# Setting a repo convention:
@desplega-bot our convention is: all API errors must return a JSON body with "error" and "code" fields
```

#### 2. "I don't care about..." - Dismiss Feedback Types

Use this phrase to suppress certain types of feedback in future reviews.

| Trigger Phrase | Effect | Scope |
|----------------|--------|-------|
| `i don't care about ...` | Lowers confidence for similar issues | Per-repo |
| `ignore ... issues` | Marks issue type as low priority | Per-repo |
| `stop flagging ...` | Suppresses similar future warnings | Per-repo |
| `don't flag ...` | Suppresses similar future warnings | Per-repo |
| `this is fine because ...` | Marks current issue as resolved | Per-thread |
| `won't fix: ...` | Marks issue as intentional | Per-thread |

**Examples:**

```markdown
# In response to a style comment:
@desplega-bot i don't care about trailing commas - our formatter handles this

# In response to a complexity warning:
@desplega-bot ignore complexity warnings in /src/legacy/ - we're not refactoring that

# In response to console.log warnings:
@desplega-bot stop flagging console.log in development scripts

# Explaining an intentional pattern:
@desplega-bot this is fine because the null check is done in the parent component

# Marking as won't fix:
@desplega-bot won't fix: we intentionally use any types in the migration files
```

#### 3. "Focus on..." - Set Review Priorities

Use this phrase to guide what the swarm should prioritize in reviews.

| Trigger Phrase | Effect |
|----------------|--------|
| `focus on ...` | Increases priority for that category |
| `focus more on ...` | Boosts confidence for that category |
| `prioritize ...` | Sets as high-importance review area |
| `care more about ...` | Increases attention to that area |
| `be stricter about ...` | Raises confidence threshold |
| `be more lenient with ...` | Lowers confidence threshold |

**Examples:**

```markdown
# Set security as high priority:
@desplega-bot focus on security issues in all API endpoints

# Prioritize performance:
@desplega-bot prioritize performance feedback for database queries

# Adjust strictness:
@desplega-bot be stricter about type safety in /src/core/

# Be more lenient:
@desplega-bot be more lenient with test file organization
```

#### 4. "In this repo..." - Set Repository-Specific Context

Use this phrase to provide context that applies to all reviews in this repository.

| Trigger Phrase | Effect |
|----------------|--------|
| `in this repo, ...` | Stores as per-repo context |
| `for this project, ...` | Stores as per-repo context |
| `we use ... in this repo` | Stores as repo convention |

**Examples:**

```markdown
# Set repo context:
@desplega-bot in this repo, we use Prisma ORM so raw SQL suggestions aren't applicable

# Define technology choices:
@desplega-bot for this project, all state management uses Zustand not Redux

# Set conventions:
@desplega-bot we use snake_case for database columns in this repo
```

### Best Practices for Memory Interaction

#### Be Specific
- **Bad**: "remember to be less strict"
- **Good**: "remember this: in /src/scripts/, we allow console.log for debugging purposes"

#### Provide Context
- **Bad**: "ignore type errors"
- **Good**: "ignore type errors in migration files - they're auto-generated by Prisma"

#### Scope Appropriately
- Use **per-repo** for standards that apply everywhere
- Use **per-PR** for one-time exceptions
- Use **per-thread** for specific decisions

#### Keep It Current
When conventions change, update the memory:
```markdown
@desplega-bot forget: the 2-space indentation rule
@desplega-bot remember this: we've switched to 4-space indentation
```

### How Memory Triggers Are Processed

```
┌─────────────────────────────────────────────────────────────┐
│                  USER COMMENT                                │
│  "@desplega-bot remember this: we use tabs not spaces"       │
└─────────────────────────────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────┐
│                  MEMORY STORAGE                              │
│  Table: pr_review_repo_memory                                │
│  Field: conventions                                          │
│  Content: { "indentation": "tabs", "source": "PR #57" }      │
└─────────────────────────────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                  CONFIRMATION RESPONSE                       │
│  "Got it! I'll remember to prefer tabs over spaces for       │
│   future reviews in this repo."                              │
└─────────────────────────────────────────────────────────────┘
```

### Querying Memory

Users can also query what the swarm remembers:

```markdown
# Show all stored preferences:
@desplega-bot what do you remember about this repo?

# Show specific category:
@desplega-bot what conventions do you have stored?

# Forget a specific memory:
@desplega-bot forget that we use tabs
```

### Confirmation Responses

When a memory trigger is detected, the swarm confirms what was stored:

| Trigger Type | Response Example |
|--------------|------------------|
| Remember | "Got it! I'll remember [X] for future reviews in this repo." |
| Dismiss | "Understood. I'll stop flagging [X] in future reviews." |
| Focus | "Noted! I'll prioritize [X] in future reviews." |
| Context | "Thanks for the context. I've stored that this repo [X]." |

### Memory Types Reference

| Memory Type | Storage | Examples | Lifetime |
|-------------|---------|----------|----------|
| **Semantic** | `pr_review_repo_memory.conventions` | Coding standards, naming rules | Permanent until updated |
| **Episodic** | `pr_review_interaction_memory` | "PR #42 had auth issue fixed" | Months |
| **Procedural** | `pr_review_global_memory.patterns` | How to review auth code | Permanent, improved over time |

### Trigger Detection Implementation

The thread response handler (Phase 4) parses these triggers:
...
const MEMORY_TRIGGERS = {
  ...
  dismiss: {
    patterns: [
      ...
      /stop flagging\s+(.+)/i,
      /don'?t flag\s+(.+)/i,
      /this is fine because\s+(.+)/i,
      /won'?t fix:\s*(.+)/i,
    ],
    action: 'dismiss',
    memoryType: 'procedural',
    importance: -0.3, // Negative to lower confidence
  },
  focus: {
    patterns: [
      /focus on\s+(.+)/i,
...
function parseMemoryTrigger(comment: string): MemoryAction | null {
  for (const [type, config] of Object.entries(MEMORY_TRIGGERS)) {
