...
**Devin provider env vars:** `DEVIN_API_KEY` (Devin provider auth, prefix `cog_*`), `DEVIN_ORG_ID` (Devin organization, prefix `org-*`), `DEVIN_POLL_INTERVAL_MS` (polling interval, default 15000), `DEVIN_ACU_COST_USD` (cost per ACU, default 2.25), `DEVIN_MAX_ACU_LIMIT` (per-session ACU cap, sent to Devin API and shown in UI budget bar), `DEVIN_REPOS` (comma-separated repos for Devin sessions), `DEVIN_API_BASE_URL` (override for testing).
