...
2. **Runner-buffered last assistant text** — the runner's provider-event loop buffers the last non-empty assistant `message` event (`trackAssistantText`). It applies the canonical secret scrubber before the 30,000-character cap (`… [truncated]` marker beyond that), so raw credentials cannot cross the task-finish boundary through this fallback. Used only when the adapter didn't populate `output` itself (`codex` today; any future adapter that emits `message` events but no `ProviderResult.output`). Empty buffer (for example `opencode`, which never emits `message` events) is a no-op — behavior is byte-identical to having no `providerOutput` at all.
