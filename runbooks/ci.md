...
| **Run Tests (1/2, 2/2)** | `bun run test:root -- --parallel=4 --shard=1/2 --timings=test-timings.json` and `--shard=2/2` | New test or test that depends on undocumented setup; a hard-coded test port colliding under `--parallel` (use `getFreePort()` / `port: 0`, see [LOCAL_TESTING.md](../LOCAL_TESTING.md)) |
