...
| **Seeded Skills Check** | `bun run check:skill-sources && bun run check:skill-md && bun run check:seed-skill-files` | Edited a generated skill source without rebuilding its `SKILL.md`, left a seeded skill unwired, or introduced a delivery-path collision |
