...
| `STEERING_DISABLE` | unset (enabled) | Set to `true` or `1` to disable new steering globally and hide its MCP/UI surfaces. Read-only message history, worker delivery callbacks already in flight, and terminal-status promotion continue so existing rows remain auditable and drain safely. |
