...
- `autoAssignPoolTasks` and `claimTask`/`assignUnassignedTaskPending` are gated by the **routing-affinity eligibility check** (§4, `isAgentEligibleForTask`) — a pooled task tagged with a `routingAffinity` snapshot (from a resume/retry, or an explicit `requiredCapabilities` on a fresh `send-task`) can only go to a role/capability-matching agent. Untagged tasks are unaffected — assignment stays open to any idle (non-lead) worker, exactly as before. Before `autoAssignPoolTasks` writes an assignee, it runs `task.before_assign` with `via=claim` outside any transaction. A hard assignment to the proposed worker proceeds; a different target or block leaves the task queued, and a block creates one Lead reroute-decision child. That creation is idempotent for the **lifetime** of the task, not merely while a decision is live (`hasRerouteDecisionChild`): a blocked pooled task stays `unassigned`, so a non-terminal-only check would let the first poll after the Lead *completes* the decision spawn another one — an endless decision loop that also duplicates the Lead's replacement work. The guard itself still re-runs per claimant on purpose, so a matcher scoped to one agent keeps rejecting that agent while leaving the task claimable by others; if nobody ever passes, the row is left to §4's pool-starvation escalation rather than terminalized here. A hard target that is unknown or is the Lead is **ignored** (fail-open to the proposed worker) — it could never claim from the pool, so honouring it would leave the task unassigned forever. Because the hook awaits, the sweep **re-reads the chosen worker's reserved count after it returns** and skips that worker if it has since reached `maxTasks`: `assignUnassignedTaskPending` only re-checks the *task* (status + affinity), never worker capacity, so the cached pre-hook count would otherwise overshoot. The final `assignUnassignedTaskPending` update still atomically re-checks `status='unassigned'`. `autoAssignPoolTasks` **does** skip idle workers whose `emptyPollCount >= MAX_EMPTY_POLLS` (the poll gate) — assigning to them would just have them exit on their next poll. The filter reads `emptyPollCount` off the rows `getIdleWorkersWithCapacity()` already returns (no per-worker re-query). Note the poll gate is cleared on a genuine `waiting_for_credentials -> ready` recovery (`updateAgentCredentialState`) and on re-register, but **not** by routine post-task `ready:true` credential reports.
...
if decision.block:
    createRoutingBlockDecisionTaskForExistingTask(task)        # idempotent for the task's LIFETIME
    skip retry child
finalAgentId = decision.assignTo ?? preferredAgentId
