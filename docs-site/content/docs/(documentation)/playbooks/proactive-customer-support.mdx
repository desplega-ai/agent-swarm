...
- **[Forward-Deployed Engineer](https://templates.agent-swarm.dev/official/forward-deployed-engineer)** (ours is named *Jackknife*) — owns engineering-level customer questions and the technical-state side of the report.

## Tools & Skills

### Built-in (ships with agent-swarm)

- **[agent-fs](https://github.com/desplega-ai/agent-fs)** — per-customer working directories. Persistent across sessions; each account accumulates institutional knowledge over months. See the [Per-Customer Working Directories pattern](./patterns/per-customer-working-directories).
...
- [**Per-Customer Working Directories**](./patterns/per-customer-working-directories) — persistent per-account context that compounds over months.
- [**HITL Gates**](./patterns/hitl-gates) — a human signs off before any direct-to-customer email.
