...
<JsonLd
  data={{
    ...
    step: [
      ...
      { "@type": "HowToStep", name: "Research", text: "Researcher agent runs /desplega:research and persists findings in agent-fs." },
      { "@type": "HowToStep", name: "Plan", text: "Plan node turns findings into an implementation plan via /desplega:create-plan." },
      { "@type": "HowToStep", name: "Implement", text: "Coder agent runs /desplega:implement-plan, opens a PR with conventional commits, and triggers CI." },
...
<Mermaid chart={`flowchart LR
  ...
  L --> R["Researcher<br/>/desplega:research"]
  R --> P["Plan<br/>/desplega:create-plan"]
...
- **[Coder](https://templates.agent-swarm.dev/official/coder)** (ours is named *Picateclas*) — turns the plan into a working PR using git worktrees, conventional commits, and the repo's existing test discipline.
...
- **[`/desplega:research` → `/desplega:create-plan` → `/desplega:implement-plan`](https://github.com/desplega-ai/ai-toolbox/tree/main/cc-plugin/base/commands)** — our open-source [Claude Code](https://www.anthropic.com/claude-code) plugin. Enforces "research before plan before code" and persists artifacts in [agent-fs](https://github.com/desplega-ai/agent-fs). Thin command wrappers in [`cc-plugin/base/commands`](https://github.com/desplega-ai/ai-toolbox/tree/main/cc-plugin/base/commands) over [`cc-plugin/base/skills`](https://github.com/desplega-ai/ai-toolbox/tree/main/cc-plugin/base/skills) (`researching`, `implementing`, …).

### Custom (swarm-managed)

- **`linear-interaction`, `linear-expert`** — outbound [Linear](https://linear.app) updates ([Linear sync is inbound-only](/docs/integrations/linear) by default — these handle the outbound side: transitioning state, posting comments, creating issues). Publishing model under [discussion](https://linear.app/desplega-labs/issue/DES-510).
...
- **`linear-drain-loop`** — given a Linear parent issue with sub-issues, branches off the previous PR's branch (stacked PRs), implements each, opens PRs, transitions Linear states. See the [Drain Loops pattern](./patterns/drain-loops).
...
- [**Drain Loops**](./patterns/drain-loops) — turn one big Linear ticket into a chain of stacked, individually-reviewable PRs.
