...
<JsonLd
  data={{
    ...
    step: [
      ...
      { "@type": "HowToStep", name: "Update docs", text: "Update docs site, CHANGELOG, last-updated timestamps; cut a GitHub release if the version lacks one." },
...
<Mermaid chart={`flowchart LR
  ...
  Q -- no --> NO["No-op (skip)"]
  Q -- yes --> UD["Update docs +<br/>CHANGELOG + release tag"]
...
- **[Coder](https://templates.agent-swarm.dev/official/coder)** (ours is *Picateclas*) — docs PRs, MDX writes, CHANGELOG bumps, release tagging.
- **[Forward-Deployed Engineer](https://templates.agent-swarm.dev/official/forward-deployed-engineer)** (ours is *Jackknife*) — [qa-use](https://github.com/desplega-ai/qa-use) captures and browser-automation for release-video recording.
...
- **`daily-docs-update`** — daily. Checks if any PRs merged in the last 24h. **No-op if none.** Otherwise updates docs + CHANGELOG + timestamps, cuts a release if needed. See the [No-op pattern](./patterns/no-op-workflows).
...
- [**No-op When Nothing Changed**](./patterns/no-op-workflows) — the daily docs job skips silently on a quiet day.
