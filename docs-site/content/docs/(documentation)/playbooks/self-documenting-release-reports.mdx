...
- **Daily** — first checks whether the version already on `main` is missing a GitHub release and cuts it if needed. Then it checks whether anything shipped in the last 24h. If yes: update the docs site, CHANGELOG, "last updated" timestamps, derive the semver bump from merged conventional commits, and open the daily release PR. The bumped GitHub release is cut by the **next** daily run after that PR lands on `main`. **If nothing shipped, no-op.**
