...
5. When `MEMORY_GRAPH_EXPANSION=1`, the candidate set is expanded with 1-hop memory-link neighbors (see [Graph Expansion](#graph-expansion))
...
| `MEMORY_GRAPH_EXPANSION` | `0` | Expand search candidates with 1-hop memory-link neighbors before reranking (`retrievalSource: "graph"` provenance). See [Graph Expansion](#graph-expansion). |
