...
- **`memory-edit`** — Edit an existing memory in place without changing its ID. Agents may edit their own memories; editing another agent's memory by ID requires a lead, even when that memory is swarm-visible. Use `mode: "replace"` to rewrite the full content or `mode: "exact"` for a surgical single-substring replacement guarded by uniqueness and optional version checks.
