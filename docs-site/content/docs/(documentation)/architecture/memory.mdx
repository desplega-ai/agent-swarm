...
4. When `MEMORY_HYBRID_SEARCH=1`, reciprocal-rank fusion blends vector matches with a full-text pass before reranking; otherwise retrieval stays vector-only
5. Candidates below the minimum similarity floor are dropped, then the survivors are **reranked** using a composite score
...
Search and retrieval events now feed a lightweight memory graph:

- **Intent-tagged retrieval rows** capture why a memory was searched or opened, not just that it was touched.
- **Typed memory links** connect memories to external artifacts such as GitHub PRs, agent-fs paths, and other entity references resolved from memory content.
...
This graph layer is additive. Core embedding search and reranking still decide which memories surface first; the edges enrich attribution and downstream tooling.
