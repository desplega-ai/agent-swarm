...
It also runs four background subsystems:
...
- **Queue stall alarm** — An API-process detector checks every five minutes for claimable `pending` or `unassigned` tasks whose oldest wait has reached 30 minutes. It alerts `SLACK_ALERTS_CHANNEL` once per stall episode and sends a recovery notice when pickup resumes, independently of the worker runner and heartbeat switch.
