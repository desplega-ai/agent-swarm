...
`CLAUDE.md` sync is a compare-and-set. Concurrent sessions of one agent share Claude's `~/.claude/CLAUDE.md`, so each session records the hash of the copy it materialized and sends it as `expectedHashes.claudeMd`. The server writes the field only if the database is still at that hash: a copy that a sibling session or an `update-profile` call already moved past is dropped instead of reverting the newer value, while an edit on top of the current value applies, including a deliberate revert to an earlier version.
