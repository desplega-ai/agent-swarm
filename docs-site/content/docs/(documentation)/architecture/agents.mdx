...
`CLAUDE.md` sync is a compare-and-set. Concurrent sessions of one agent share Claude's `~/.claude/CLAUDE.md`, and each session start and end rewrites it, so the hook keeps a lineage record for the file: what a hook last wrote there, and which database value that content derives from. Content a hook wrote is never synced back. An agent's edit is sent with its base as `expectedHashes.claudeMd`, and the server writes the field only if the database is still at that value — a copy another session or an `update-profile` call already moved past is dropped (recorded as a `system.profile_sync_conflict` event) instead of reverting the newer value, while an edit on top of the current value applies, including a deliberate revert to an earlier version.

`HEARTBEAT.md` also validates new tracked lines at profile-write time. Only the section after the last `## Tracked Items` marker is checked (excluding the marker line, ending at the next level-one or level-two heading). New items require `DELETE MM-DD` or `DELETE on MM-DD`, case-insensitively. Lines beginning with 📌, optionally after a Markdown list/checklist prefix, are permanent and exempt.

Existing undated lines may remain or be deleted. A replacement is recognized as a legacy edit when it keeps the opening bold Markdown title or only deletes text from the old line. Matches are one-to-one, so keeping an old item does not authorize another undated copy. A complete rewrite without a retained title needs an expiry because plain text has no stable item IDs. Rejection leaves the profile transaction unchanged and returns HTTP 400 or an MCP error naming the offending line; filesystem sync logs the server error.
