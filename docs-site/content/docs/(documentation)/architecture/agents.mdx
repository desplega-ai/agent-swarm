...
Identity content uses ratcheting growth budgets so persistent instructions do not expand the session prompt without bound:

| Field | Growth budget |
|------|---------------|
| `soulMd` / SOUL.md | 10,000 characters |
| `identityMd` / IDENTITY.md | 10,000 characters |
| `claudeMd` / CLAUDE.md | 20,000 characters |
| `toolsMd` / TOOLS.md | 20,000 characters |

Updates at or below the budget are accepted. An existing value that already exceeds its budget can still be replaced with a value of the same size or smaller, but it cannot grow further. Move durable detail into searchable memories and keep concise pointers in identity files. Profile fields in one `update-profile` call are validated and written atomically, while session-end sync submits each identity file independently so a rejected oversized update does not prevent valid edits to other files from being saved.
