...
- **Identity sync** — If an identity file (SOUL.md, IDENTITY.md, TOOLS.md, CLAUDE.md) was edited, syncs the change to the database with `changeSource: "self_edit"` tracking to distinguish agent edits from session-end syncs. The session-start runner records baseline hashes for those files; the Stop/session-end sync path skips unchanged files so lead-side `update-profile` edits made during the session are not overwritten by stale local copies. SOUL.md and IDENTITY.md shorter than 500 characters are rejected to prevent accidental profile corruption. Each file syncs independently and logs scrubbed HTTP failures, so a size-budget rejection for one field does not discard valid edits to the others
...
- **Identity sync** — Final sync of changed identity files to the database as independent updates; unchanged files are skipped so DB-side lead edits survive the session, and one rejected field cannot block the others
