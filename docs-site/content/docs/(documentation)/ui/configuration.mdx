...
The **Configuration** page (Settings → Configuration, at `/settings/configuration` in the dashboard) surfaces the environment variables that shape how your swarm behaves — add-ons, steering, memory retrieval, heartbeat and crash recovery, harness selection, integration toggles, security, workflow limits, and branding — as editable settings, so you can tune a running deployment without shelling into the host to edit `.env` files.
...
| Group | Examples |
| --- | --- |
| Add-ons | `DREAMING_ENABLED`, `DREAMING_SLACK_CHANNEL` |
