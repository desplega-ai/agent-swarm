...
`CORS_ALLOW_ANY_ORIGIN` is deployment-only and is not available in Configuration. Set it in the API deployment environment and restart the API if unrestricted credentialed CORS is intentionally required. HTTP, MCP, and database config writes reject this reserved key; legacy stored rows are ignored during startup and reload and can be deleted. Prefer configuring trusted origins with `CORS_ALLOWED_ORIGINS`.
