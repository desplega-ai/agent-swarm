...
The Agent Swarm **dashboard** is the web UI for operating a swarm: watch agents, tasks, and inter-agent chat in real time, review approvals, run [Swarm Apps](/docs/concepts/apps), manage workflows, scripts, schedules, skills, and memory, and administer settings — secrets, API keys, integrations, and [swarm configuration](/docs/ui/configuration). Complex skills expose their bundled file tree in the skill detail view, with rendered Markdown and read-only source previews alongside the canonical `SKILL.md`.
