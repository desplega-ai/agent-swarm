...
| `foreach` | Fan out one `agent-task` child per array item and join their results |
...
### Data-Driven Fan-Out with `foreach`

Use a `foreach` node when the fan-out width comes from runtime data rather than a fixed set of `next` branches. In v1, each item creates one `agent-task` child; the parent waits for every child and exposes a single aggregate to its successor.

```json
{
  "id": "review-each",
  "type": "foreach",
  "inputs": { "files": "discover.result.files" },
  "config": {
    "over": "{{files}}",
    "itemKey": "path",
    "body": {
      "type": "agent-task",
      "config": {
        "template": "Review {{item.path}} (item {{index}})"
      }
    }
  },
  "next": "summarize"
...
`over` must resolve to an array, and `itemKey` must name a unique, non-empty string field on each item. The body is restricted to `agent-task` in v1 and can interpolate `item` plus the zero-based `index`. Empty arrays complete normally; `concurrency`, nested loops, and `foreach` nodes inside graph cycles are rejected. Child IDs use `<foreachNodeId>#<itemKey>`, while downstream nodes receive the parent aggregate with `results`, `okCount`, and `failedCount`.

The workflow-level `onNodeFailure` policy also governs foreach children: `fail` stops on a failed or cancelled child, while `continue` records the failed result and lets the remaining children close the join.
