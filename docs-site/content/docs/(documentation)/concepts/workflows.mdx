...
`questions` can also be built at run time: set it to one exact token such as `"{{plan.questions}}"` (with a matching `inputs` mapping) and the upstream array becomes the card's questions, for example one `single-select` per item. The array is validated when the node runs; a missing, empty, or malformed value (or more than 100 questions) fails the node without creating a card.
