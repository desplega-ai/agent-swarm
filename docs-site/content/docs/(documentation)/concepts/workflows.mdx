...
The engine includes 11 built-in executor types:
...
For `swarm-script` nodes there is a second timeout surface inside `config.timeoutMs`. Use the node-level `timeoutMs` for general step orchestration limits and the config-level `timeoutMs` to raise or lower the spawned script's own wall-clock budget within the allowed `1000`-`60000` ms range.

Inline `script` nodes use `config.timeout` instead. That value is the wall-clock budget for both the inline executor and the workflow step watchdog, so a long-running script is not stopped by the default 30-second watchdog before its configured timeout. The minimum is `1000` ms and the default is `30000` ms.
