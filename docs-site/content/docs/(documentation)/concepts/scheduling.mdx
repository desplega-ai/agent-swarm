...
The call completes the current task and schedules one continuation for the same agent. `settled` wakes on completion or failure; use `task.completed` or `task.failed` to watch only that outcome. The required delay or `runAt` is the latest scheduled wake-up time if the event does not arrive. Cancellation is not a `settled` event. An already-terminal watched task is rejected so you can read its result immediately.
