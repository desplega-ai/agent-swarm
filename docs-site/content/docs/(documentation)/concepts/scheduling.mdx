...
The call completes the current task and schedules one continuation for the same agent. `settled` wakes on completion or failure; use `task.completed` or `task.failed` to watch only that outcome. The required delay or `runAt` is the latest scheduled wake-up time if the event does not arrive. Cancellation is not a `settled` event.
...
`mode` defaults to `all`: every member must match the requested event. Set `mode: "any"` to wake on the first matching member. For example, `all` with `settled` accepts a mix of successes and failures; `all` with `task.completed` cannot wake early if one member fails. The single deadline bounds the whole set.
