...
For longer waits, use [defer-task](/docs/concepts/scheduling#deferring-until-another-task-finishes) to complete the current task and book a continuation. Delegated tasks create a lead follow-up on completion or failure by default. The default lead prompt reserves inline polling for essential results expected within about a minute; that path sets `followUpConfig.disabled: true` to avoid an extra follow-up and defers if the wait runs longer.
