...
### Crash Recovery — Same-Agent Pin + Lead Fallback

When the heartbeat classifies a task as crashed (its worker has gone unresponsive), the recovery `resume` task is **pinned back to the original agent** instead of being released to the role-blind unassigned pool. Agent IDs are stable across a restart and the crashed agent's row survives, so the resume is reclaimed when that same agent comes back — and no wrong-specialization worker can pick it up in the meantime.
...
<Callout type="info">
  Two environment variables gate this behavior:
