...
### Queue-Pickup Stall Alarm

The API process also monitors the other side of liveness: tasks that are ready to run but have not been claimed. Every five minutes it checks claimable `pending` and `unassigned` tasks, excluding tasks whose dependencies are incomplete. If the oldest claimable task has waited at least 30 minutes, the API posts a deduplicated alert to `SLACK_ALERTS_CHANNEL` with the queue size, oldest task, wait time, and recent pickup count. It sends one recovery notice when the queue begins moving again.

This alarm starts independently of the worker runner and remains active when `HEARTBEAT_DISABLE` is set, so detecting a pickup outage does not itself require an agent to claim a monitoring task.
