...
```
send-task(task: "Fix the login bug", agentId: "worker-uuid", routingReason: "skill", routingNote: "Owns the authentication module")
```
...
```
send-task(task: "Refactor auth module", agentId: "worker-uuid", routingReason: "skill", routingNote: "Owns the authentication module", offerMode: true)
```
...
An explicit `send-task.agentId` requires both `routingReason` and a `routingNote` explaining why that worker fits. The note must contain at least 10 characters after trimming whitespace and at most 200 characters. Choose `skill`, `continuity`, `overflow`, `human_pinned`, or `reroute_fault` as the reason. REST callers must supply `routingReason` when `POST /api/tasks` specifies either `agentId` or `offeredTo`; the REST note remains optional. Pool submissions omit the reason. If `send-task` inherits its assignee from `parentTaskId`, it records `continuity` automatically without requiring a note.

Task responses include the recorded reason, note, and `routingSource`: `declared` for a caller's choice or `engine_default` for an automatic routing decision. The source can be absent for historical tasks or tasks without a reason. At task creation, the server also attempts to persist a decision-time snapshot of worker status and load, plus the most recent continuity candidates from the same PR, thread, and repository. Snapshot failures are logged without blocking task creation.
...
For longer waits, use [defer-task](/docs/concepts/scheduling#deferring-until-another-task-finishes) to complete the current task and book a continuation. Delegated tasks create a lead follow-up on completion or failure by default. A settlement that wakes a `defer-task` wait for the same agent skips that follow-up; see [scheduling](/docs/concepts/scheduling#deferring-until-another-task-finishes). The default lead prompt reserves inline polling for essential results expected within about a minute; that path sets `followUpConfig.disabled: true` to avoid an extra follow-up and defers if the wait runs longer.
