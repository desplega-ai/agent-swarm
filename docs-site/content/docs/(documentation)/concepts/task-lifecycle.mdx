...
```
send-task(task: "Fix the login bug", agentId: "worker-uuid", routingReason: "skill")
```
...
```
send-task(task: "Refactor auth module", agentId: "worker-uuid", routingReason: "skill", offerMode: true)
```
...
An explicit `send-task.agentId` requires `routingReason`. REST callers must also supply it when `POST /api/tasks` specifies either `agentId` or `offeredTo`. Choose `skill`, `continuity`, `overflow`, `human_pinned`, or `reroute_fault`; optionally add `routingNote` with up to 200 characters. Pool submissions omit the reason. If `send-task` inherits its assignee from `parentTaskId`, it records `continuity` automatically.

Task responses include the recorded reason and note. At task creation, the server also attempts to persist a decision-time snapshot of worker status and load, plus the most recent continuity candidates from the same PR, thread, and repository. Snapshot failures are logged without blocking task creation.
