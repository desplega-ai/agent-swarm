...
| `routingNote` | string | With `agentId` | Why this worker fits the task: at least 10 characters after trimming whitespace, at most 200 characters |
...
| `citations` | array | No | Claim sources, upserted by index across calls. Use only for factual claims the reader cannot already see in the thread or task tree, 1-2 sources; none on delegation, routing, acks, or status replies. Reference each in `output` with `[citation:N]`, or set `general: true` for a source that backs the whole answer. Kinds: `task`, `memory`, `github`, `slack`, `agent-fs`, `page`, `script-run`, `url`. The first completing call is refused (the task stays in progress) if a marker has no entry, a citation fails validation, or a non-general citation is unreferenced; after one refusal, completion proceeds and those markers are dropped. At most 50 per call/task. |
...
Pass `taskId`, one of `delayMs` or `runAt`, a `summary` of what you did and where things stand, a `note` that says what is pending, and optional `checks` to verify on wake-up. The task reaches its final state (`completed`) on this call and receives a `deferred` tag. A one-off schedule targets you, and its child task carries the deferred task as `parentTaskId` so the wake-up run receives the prior context. Use this tool instead of creating a continuation schedule by hand.

- **Early wake-up:** optionally pass `wakeOn: { event: "settled", taskId: "<watched-task-id>" }`, or use `taskIds: ["<first-id>", "<second-id>"]` instead of `taskId`. `mode` defaults to `all` (every member must match); use `mode: "any"` for the first match. Events are `task.completed`, `task.failed`, or `settled` (completed, failed, or cancelled). Cancellation does not match either named event. IDs must be unique, exist, be nonterminal, and differ from the task being deferred. Any invalid or already-terminal member rejects the entire call without completing the task or creating a schedule. One `delayMs` or `runAt` deadline is still required for the whole set; events and the deadline share a single wake-up so racing triggers do not create duplicate children. Pending waits survive API restarts. The member whose settlement wakes you creates no separate lead follow-up for your agent unless its `followUpConfig` sets `onCompleted`/`onFailed`; an `all`-mode member that settles while others are pending still does.
- **Structured output:** if the task has an `outputSchema`, pass `output` as a JSON string matching it. The tool stores that string verbatim as terminal output and puts the summary and deferral details in the task log. Missing or invalid output is rejected before creating a schedule. Without a schema, terminal output says when the agent will check back, or names the agents it is waiting on and the latest wake-up time; `output` is ignored. The time uses the requester's stored timezone, falling back to a labeled UTC time when none is set. Full summary, checks, and schedule details remain in the task log.

See [deferring until another task finishes](/docs/concepts/scheduling#deferring-until-another-task-finishes) for an example.

If a watched member defers, that completion does not count as a match. The member follows its resume task across further deferrals without changing the set's ceiling. Disabling or deleting a successor schedule before it runs makes the parent's completion eligible on the next reconciliation; a wait for `task.failed` still falls back to its ceiling.

Superseded members follow their `resume` children without waking or extending the ceiling, regardless of whether supersession or child creation happens first. If no resume child exists, the wait holds to its original ceiling. Supersession itself never matches an event.
...
The `scripts` capability includes tools for trusted TypeScript hook bundles. Extensions run
inside the API server process. See the [extensions guide](/docs/guides/extensions) for the
bundle format, event contracts, and lead/operator activation workflow.

### extension-list

List installed extensions with their version, activation state, priority, failure count,
and owner (`createdByAgentId`). The owner differs from the extension runtime `agentId`.
...
List the predefined extensions that can be installed, with their declared assets and
installed state. The catalog (also `GET /api/extensions/catalog`) is the only install source.

*No parameters.*

### extension-install

Install a predefined extension from the catalog by name. Inline bundles are rejected.
Install creates the extension, its `ext:<name>` agent, and every asset it declares
(scripts, and disabled schedules). Any authenticated agent can install; the installing
agent becomes its owner (`createdByAgentId`), and workers may install subsequent versions
only for their own extensions, without activating them. A trusted lead can enable the new
extension with `extension-enable`; operators and dashboard users can also activate it.
Installation itself never enables an extension.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `template` | string | Yes | Name of a predefined extension from `extension-catalog` |
| `priority` | integer | No | Handler priority; lower values run first |
| `config` | object | No | Extension configuration |

### Extension lifecycle

Enable, disable, and version activation require a lead, operator, or dashboard user.
Workers can delete their own disabled extensions, but cannot activate any extension
or modify another owner's bundle.
`EXTENSION_ALLOW_LEAD_ACTIVATION=false` restricts enable, disable, and version activation
to operators/dashboard users; leads can still install drafts and delete disabled extensions.
Extensions execute inside the API process without host isolation.

| Tool | Required parameters | Behavior |
|------|---------------------|----------|
| `extension-enable` | `id` (UUID) | Load and enable an installed extension |
| `extension-disable` | `id` (UUID) | Disable an extension and remove its registered hooks |
| `extension-activate-version` | `id` (UUID), `version` (positive integer) | Activate a stored version, reloading it if enabled |
| `extension-delete` | `id` (UUID) | Delete a disabled extension and its stored history; disable it first |

Get the installed `id` from `extension-list`. The script SDK exposes these tools with
underscores, for example `ctx.swarm.extension_enable({ id })`.
