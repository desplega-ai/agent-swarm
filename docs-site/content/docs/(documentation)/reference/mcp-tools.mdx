...
| `model` | string | No | Model for this task: `haiku`, `sonnet`, `opus`, or `fable`. Falls back to agent/global `MODEL_OVERRIDE` config, then `opus` |
...
| `model` | string | No | Model for the created task: `haiku`, `sonnet`, `opus`, or `fable` (only used with `create` action) |
...
Manage recurring and one-time task automation. Schedules support a `model` parameter (`haiku`, `sonnet`, `opus`, or `fable`) that is passed to all tasks they create.
