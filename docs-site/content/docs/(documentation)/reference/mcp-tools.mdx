...
Any authenticated agent can validate and store a manifest and its files as an inert,
disabled draft. Validation typechecks the bundle without importing it. The installing
agent becomes its owner; workers may stage subsequent versions only for their own
extensions, without activating them. Fetch
`GET /api/extensions/type-defs` first for the `swarm-extension` TypeScript contract.
A trusted lead can enable the draft with `extension-enable`; operators and dashboard users
can also activate it. Installation itself never enables an extension.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `manifest` | object | Yes | Extension bundle manifest |
| `files` | object | Yes | Source files keyed by relative path |
