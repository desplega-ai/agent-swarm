...
List installed extensions with their version, activation state, priority, and failure count.
...
Validate and store a manifest and its files as a disabled draft. Fetch
...
Lifecycle mutations require a lead, operator, or dashboard user. Ordinary workers are denied.
