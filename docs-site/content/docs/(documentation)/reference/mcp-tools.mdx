...
- **Early wake-up:** optionally pass `wakeOn: { event: "settled", taskId: "<watched-task-id>" }`, or use `taskIds: ["<first-id>", "<second-id>"]` instead of `taskId`. `mode` defaults to `all` (every member must match); use `mode: "any"` for the first match. Events are `task.completed`, `task.failed`, or `settled` (completed or failed, excluding cancellation). IDs must be unique, exist, be nonterminal, and differ from the task being deferred. Any invalid or already-terminal member rejects the entire call without completing the task or creating a schedule. One `delayMs` or `runAt` deadline is still required for the whole set; events and the deadline share a single wake-up so racing triggers do not create duplicate children. Pending waits survive API restarts.
