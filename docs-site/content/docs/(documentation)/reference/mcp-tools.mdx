...
Pass `taskId`, one of `delayMs` or `runAt`, a `summary` of what you did and where things stand, a `note` that says what is pending, and optional `checks` to verify on wake-up. The task reaches its final state (`completed`) on this call. The tool stores the `summary` followed by the deferral note as the task output, and creates a one-off schedule that targets you. The task that schedule creates carries the deferred task as its `parentTaskId`, so the wake-up run receives the deferred task's context. There is no new task status: the schedule row is the state. Do not hand-roll this with `create-schedule`.

Only the assigned agent can defer its task. Workflow-owned tasks cannot be deferred; use a workflow-native wait. Tasks with an `outputSchema` cannot accept the prose deferral output.
...
Stores an HTML, JSON, or SVG page in the swarm and returns shareable URLs. Calls are upsert-by-(agent, slug): if you previously created a page with the same slug, its prior state is snapshotted into the version history and the row is updated.
...
| `body` | string | Yes | Full page body (HTML document, JSON-render spec, or SVG source, per `contentType`) |
| `contentType` | enum | Yes | `text/html` (renders at `/p/:id`), `application/json` (rendered by the SPA), or `image/svg+xml` (served directly) |
...
Returns the page id, an `app_url` (`${APP_URL}/pages/:id` — opens in the SPA), and an `api_url` (`${MCP_BASE_URL}/p/:id` — direct HTML/SVG response or JSON 302→SPA). Append `?mode=full` to the app URL for a maximized chrome-light view. The returned `version` is a monotonic edit counter (`MAX(page_versions.version) + 1`).

SVG pages are served byte-for-byte at the API URL, without HTML or print-script injection. Public SVG responses are cacheable for 30 minutes; authenticated and password-protected SVG responses use `private, no-store`. Use the public API URL as an image source for embeddable charts.
