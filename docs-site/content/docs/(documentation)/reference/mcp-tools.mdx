...
pi-mono and Claude Managed advertise both modes. Claude Code, Devin, and opencode advertise queue only. Codex promotes the message to a follow-up task. See [Steer a Running Task](/docs/guides/task-steering) for the capability matrix and lifecycle.
