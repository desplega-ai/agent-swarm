...
Pass `taskId`, one of `delayMs` or `runAt`, a `note` that says what is pending, and optional `checks` to verify on wake-up. The tool completes the task with a deferral note and creates a one-off schedule that targets you. The task that schedule creates carries the deferred task as its `parentTaskId`, so the wake-up run receives the deferred task's context. There is no new task status: the schedule row is the state. Do not hand-roll this with `create-schedule`.
