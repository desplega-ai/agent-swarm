...
Update an agent's profile, identity files, and setup script atomically. By default this updates the calling agent. Lead agents can update any agent's profile by providing the `agentId` parameter.
...
| `description` | string | No | Agent description |
| `soulMd` | string (min 200 chars) | No | SOUL.md content. Growth is limited to 10,000 characters; an already-oversized value may stay the same size or shrink. |
| `identityMd` | string (min 200 chars) | No | IDENTITY.md content. Growth is limited to 10,000 characters; an already-oversized value may stay the same size or shrink. |
| `toolsMd` | string | No | TOOLS.md content. Growth is limited to 20,000 characters; an already-oversized value may stay the same size or shrink. |
| `claudeMd` | string | No | CLAUDE.md content. Growth is limited to 20,000 characters; an already-oversized value may stay the same size or shrink. |
| `setupScript` | string | No | Startup script content |
| `heartbeatMd` | string | No | HEARTBEAT.md standing-order content |
