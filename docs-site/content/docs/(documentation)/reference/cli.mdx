---
title: CLI Reference
description: Complete CLI reference for managing agents, tasks, and swarm configuration from the terminal — install via bunx, manage worktrees, run agent commands, and configure your swarm without the dashboard UI
---
...
Build Agent Swarm E2B templates and start API or worker sandboxes on demand for CI and Dockerless smoke tests.
...
bunx @desplega.ai/agent-swarm e2b start-stack --workers 2 --api-key "$SWARM_API_KEY"
```
...
| `start-stack` | Start an API plus one or more workers |
| `list` | List active E2B sandboxes |
| `kill <sandbox-id...>` | Stop one or more E2B sandboxes |
