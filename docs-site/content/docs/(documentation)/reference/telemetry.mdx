...
| `task.created` | A new task is created | source, hasParent, has_repo, priority |
...
| `session.failure` | A provider session fails without an intentional cancellation | agentId, provider, model, errorCategory, durationMs, wasRateLimited |
| `session.cancelled` | A provider reports an intentional cancellation | agentId, provider, model, errorCategory, durationMs, wasRateLimited |
...
| `integration.connected` | An integration is connected | type, provider?, first_of_type |
...
| `onboarding.started` | The API creates or replaces onboarding state | existing_install, seconds_since_start, seconds_since_install? |
| `onboarding.step_viewed` | The active setup step changes | step, seconds_since_start, seconds_since_install? |
| `onboarding.step_completed` | A setup step completes from a live signal or explicit action | step, method?, derived, seconds_since_start, seconds_since_install? |
| `onboarding.step_skipped` | The operator skips an optional setup step | step, seconds_since_start, seconds_since_install? |
| `onboarding.step_failed` | A setup step records a classified failure | step, error_class, seconds_since_start, seconds_since_install? |
| `onboarding.dismissed` | The operator dismisses onboarding | seconds_since_start, seconds_since_install? |
| `onboarding.first_task_completed` | The task selected during setup completes | method?, seconds_since_start, seconds_since_install? |
| `onboarding.completed` | The first setup task completes the onboarding funnel | seconds_since_start, seconds_since_install? |
