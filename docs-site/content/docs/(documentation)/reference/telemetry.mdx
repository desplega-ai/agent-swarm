...
- `install_preset` (metadata, optional) — the onboard wizard preset used (e.g. `solo`), when the install went through the wizard.
- `install_created_at` (metadata, optional) — ISO timestamp of when this install's identity was first minted, persisted once alongside the installation ID. Lets analysis reconstruct the time from install to first real task without depending on server-restart timing.
...
A `telemetry_installed_at` timestamp is minted alongside the installation ID the same way (and backfilled once, on first upgrade, for installs that predate this field) — it anchors "when this install came into being" independent of server restarts.
