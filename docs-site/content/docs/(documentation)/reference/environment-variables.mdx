...
| `SWARM_USE_CLAUDE_BRIDGE` | No | `false` | Reloadable boolean (`true`/`1` enables, `false`/`0`/unset disables). When enabled, the Claude adapter routes through the installed `claude-bridge` binary from pinned package `@desplega.ai/claude-bridge@0.1.10`, a Desplega-owned `claude -p` drop-in that drives interactive Claude Code through `tmux`. Requires Bun, `claude`, and `tmux` on PATH. See [Claude Bridge](/docs/guides/claude-bridge-experimental). |
