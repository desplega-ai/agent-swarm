...
| `BEDROCK_AUTH_MODE` | No | inferred from `MODEL_OVERRIDE` | Set `sdk` for the AWS credential chain or `bearer` for a Bedrock API key. Both modes probe credentials and enumerate available models. When unset, an `amazon-bedrock/*` model override activates SDK mode |
| `AWS_BEARER_TOKEN_BEDROCK` | In bearer mode | — | Bedrock API key required by `BEDROCK_AUTH_MODE=bearer`; other provider API keys do not satisfy this check |
| `AWS_REGION` | In either Bedrock mode | — | Required region for credential probing and account-accurate model enumeration in SDK, bearer, or inferred SDK mode |
