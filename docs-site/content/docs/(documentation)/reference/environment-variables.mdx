...
| `SLACK_ALERTS_CHANNEL` | Slack channel ID/name for OAuth/Jira keepalive failures and claimable queue-stall/recovery alerts. Unset means integration failures are logged only; queue-alarm delivery is retried on its next check |
