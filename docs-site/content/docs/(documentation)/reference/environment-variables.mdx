...
| `CORS_ALLOWED_ORIGINS` | Hosted/dev allowlist (below) | Comma-separated exact origins or wildcard-subdomain patterns. Unset or blank uses built-in defaults; a nonblank value replaces them. Unlisted origins receive neither Allow-Origin nor Allow-Credentials. Applies to API and page-session endpoints. |
| `CORS_ALLOW_ANY_ORIGIN` | `false` | Explicit compatibility opt-out: reflect any origin with credentials, overriding the allowlist. Warns once per process when enabled. Prefer listing trusted SPA origins instead. |
...
If you intentionally need the previous unrestricted behavior, set `CORS_ALLOW_ANY_ORIGIN=true`.
This overrides even a configured allowlist and allows arbitrary sites to read credentialed
responses when the browser sends a page-session cookie. It is an explicit compatibility option.
Both settings take effect after Configuration reload; environment changes require restarting the API.
The API warns once per process at startup, or on first use after enabling the opt-out via reload.
