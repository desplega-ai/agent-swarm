...
| `SWARM_DEP_REDIS_ENABLED` | No | `false` | Set to `true` to start the optional bundled Redis helper on worker boot (entrypoint runs `scripts/init-local-redis.sh`). Redis binaries ship dormant in the worker image since `v1.99.0` |
| `LOCAL_REDIS_DATA_DIR` | No | `/tmp/redis-data` | Data directory for the optional local Redis instance |
| `LOCAL_REDIS_PORT` | No | `6379` | Port for the optional local Redis instance |
