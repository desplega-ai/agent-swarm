...
|----------|----------|---------|-------------|
| `HARNESS_PROVIDER` | No | `claude` | AI provider: `claude` (Claude Code), `codex`, `pi` (pi-mono), `devin` (Devin), `claude-managed` (Anthropic Managed Agents), or `opencode` (opencode SDK). See [Harness Configuration](/docs/guides/harness-configuration) |
...
| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `OPENROUTER_API_KEY` | One of* | — | OpenRouter API key (preferred when present) |
| `ANTHROPIC_API_KEY` | One of* | — | Anthropic API key |
| `OPENAI_API_KEY` | One of* | — | OpenAI API key |

\* At least one API key OR a populated `~/.local/share/opencode/auth.json` is required. `validateOpencodeCredentials` checks them in priority order (`OPENROUTER_API_KEY` → `ANTHROPIC_API_KEY` → `OPENAI_API_KEY` → auth file) and fails fast at boot if none are found. The opencode adapter is currently being rolled out across DES-295 → DES-299 — treat as experimental.
