...
|----------|----------|---------|-------------|
| `HARNESS_PROVIDER` | No | `claude` | AI provider: `claude` (Claude Code), `codex`, `pi` (pi-mono), or `devin`. See [Harness Configuration](/docs/guides/harness-configuration) |
...
| `DEVIN_MAX_ACU_LIMIT` | No | — | Per-session ACU cap. Sent to the Devin API and shown in the UI budget bar |
| `DEVIN_ACU_COST_USD` | No | `2.25` | Cost per ACU in USD for cost reporting |
| `DEVIN_API_BASE_URL` | No | `https://api.devin.ai` | API base URL override for testing or on-prem |

Devin sessions run fully remote — no local CLI, MCP, or filesystem access. Repos are specified via the task's `vcsRepo` field. See [Harness Configuration](/docs/guides/harness-configuration#devin) for details.
...
- [Harness Configuration](/docs/guides/harness-configuration) — Configure Claude Code, Codex, pi-mono, or Devin provider settings
