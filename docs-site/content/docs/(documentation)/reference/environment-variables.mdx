...
| `HEARTBEAT_UNCLAIMED_STALL_MIN` | `15` | Minutes before an unclaimed task records a visible stall reason |
| `HEARTBEAT_UNCLAIMED_FAIL_MIN` | `60` | Total minutes before an unclaimed task fails after receiving its warning window |
| `HEARTBEAT_IN_PROGRESS_HARD_STOP_MIN` | `120` | Minutes without task progress before a live-session stall is failed |
