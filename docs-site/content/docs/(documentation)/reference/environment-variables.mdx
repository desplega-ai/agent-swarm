...
| `CORS_ALLOWED_ORIGINS` | — (reflects any origin) | Comma-separated exact origins or wildcard-subdomain patterns (e.g. `https://*.agent-swarm.dev`) allowed to receive credentialed CORS headers (`Access-Control-Allow-Credentials: true`). Unset preserves the default reflect-any-origin behavior most SPA deployments rely on today. Set it to restrict which origins can make credentialed cross-origin requests against the API, including through the page-session cookie proxy (`/@swarm/api/*`). See [Built-in API CORS](https://github.com/desplega-ai/agent-swarm/blob/main/DEPLOYMENT.md#built-in-api-cors). |
...
Recommended for our hosted deployments:

```bash
CORS_ALLOWED_ORIGINS=https://*.agent-swarm.dev,https://*.agent-swarm.cloud
```

For local development, append `,http://localhost:5274` (the dashboard dev server).
This is a recommendation, not an enforced default. Unset or blank still reflects any origin;
operators opt into restrictions by setting the variable explicitly.
