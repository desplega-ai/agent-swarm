...
| `CORS_ALLOWED_ORIGINS` | — (reflects any origin) | Comma-separated exact origins (e.g. `https://app.example.com`) allowed to receive credentialed CORS headers (`Access-Control-Allow-Credentials: true`). Unset preserves the default reflect-any-origin behavior most SPA deployments rely on today. Set it to restrict which origins can make credentialed cross-origin requests against the API, including through the page-session cookie proxy (`/@swarm/api/*`). See [Built-in API CORS](https://github.com/desplega-ai/agent-swarm/blob/main/DEPLOYMENT.md#built-in-api-cors). |
