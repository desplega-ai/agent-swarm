...
| `CORS_ALLOW_ANY_ORIGIN` | `false` | Deployment-only compatibility opt-out (requires API restart; cannot be stored in Configuration): reflect any origin with credentials, overriding the allowlist. Warns once per process when enabled. Prefer listing trusted SPA origins instead. |
