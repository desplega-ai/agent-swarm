...
|----------|----------|---------|-------------|
| `HARNESS_PROVIDER` | No | `claude` | AI provider: `claude` (Claude Code), `codex`, `opencode`, `pi` (pi-mono), `devin` (Devin), `claude-managed` (Anthropic Managed Agents), or `acp` (any ACP-compatible agent). See [Harness Configuration](/docs/guides/harness-configuration) |
...
| `SWARM_USE_CLAUDE_BRIDGE` | No | `false` | Reloadable boolean (`true`/`1` enables, `false`/`0`/unset disables). When enabled, the Claude adapter routes through the installed `claude-bridge` binary from pinned package `@desplega.ai/claude-bridge@0.1.8`, a Desplega-owned `claude -p` drop-in that drives interactive Claude Code through `tmux`. Requires Bun, `claude`, and `tmux` on PATH. See [Claude Bridge](/docs/guides/claude-bridge-experimental). |
...
**ACP** (`HARNESS_PROVIDER=acp`):

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `ACP_TARGET` | No | `custom` | Target selector: `gemini`, `claude-agent-acp`, `codex-acp`, or `custom` |
| `ACP_TARGET_COMMAND` | Conditional | — | Executable for `custom` target. Required when `ACP_TARGET=custom` (the default) |
| `ACP_TARGET_ARGS` | No | — | Extra arguments for `ACP_TARGET_COMMAND`. JSON array or whitespace-separated string |
| `ACP_SYSTEM_PROMPT_PATH` | No | — | File path where the adapter writes the system prompt (for targets that read it from disk) |
| `GOOGLE_API_KEY` | Conditional | — | Required for `ACP_TARGET=gemini` (or `GEMINI_API_KEY`) |
| `GEMINI_API_KEY` | Conditional | — | Alternative to `GOOGLE_API_KEY` for the gemini target |

When `ACP_TARGET=claude-agent-acp`, use the same `CLAUDE_CODE_OAUTH_TOKEN` / `ANTHROPIC_API_KEY` credentials as the `claude` harness. When `ACP_TARGET=codex-acp`, use `OPENAI_API_KEY` or `~/.codex/auth.json`. See [Adding a Harness Provider § ACP](/docs/guides/harness-providers).
