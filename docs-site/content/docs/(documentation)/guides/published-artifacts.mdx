...
Both variants ship all four harness CLIs (Claude Code, Codex, pi, opencode), the compiled `agent-swarm` binary, Node.js, Bun, Python, git, `gh` with the official `gh stack` extension, `pm2`, the `agent-fs` and `wts` CLIs, and the full skill trees for every harness.
