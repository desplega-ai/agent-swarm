...
For delegated tasks tracked in a task tree, output that would be truncated in
the compact tree preview is delivered in full to the originating thread before
the terminal tree is cleaned up. Long results are converted to Slack mrkdwn and
split into safe Block Kit chunks across as many messages as needed. Successful
delivery marks the task as replied so the completion watcher does not duplicate
it; if delivery fails, tree cleanup waits and retries.
