...
For a tracked task-trigger message, the acceptance reaction is temporary. Once every task linked to that trigger is terminal, the swarm removes its temporary :eyes:, :heavy_plus_sign:, or :zap: reaction, then adds :white_check_mark: if every linked task completed or :x: if any task failed or was cancelled. Steering acknowledgements and later additive-buffer reactions are not task triggers, so they remain unchanged.
