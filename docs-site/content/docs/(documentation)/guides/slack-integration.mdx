...
With `SLACK_RENDER_V2=true` (the default), each Slack thread owns exactly one engine message: a task tree that is updated in place with `chat.update`. Later asks append to the same tree, delegated tasks stay nested under their parent, and task IDs remain clickable:

```
🧵 Thread title · 44m
├─ ✅ First ask · 7m51s · task-id
├─ ⏳ Current ask · 8m05s · task-id
│  └─ ✅ Researcher · 8m24s · task-id
└─ ✅ Completed ask · 12m · task-id → result
```

The tree contains status, elapsed time, hierarchy, task links, and links to completed results. It does not contain worker output or a footer. Updates are debounced and rate-limit retries honor Slack backoff.

### Outcome Cards and Message Provenance

Each completed Slack ask gets one compact outcome card. The renderer writes it once with `chat.startStream`, about three `chat.appendStream` calls, and `chat.stopStream`; it is never passed to `chat.update`, so Slack does not stamp it as edited. The card links back to the tree, and the tree links to the card.
...
### Roll Back the Renderer

Set `SLACK_RENDER_V2=false` to restore the legacy per-task assignment/progress/completion renderer without reverting the deployment.

### Rich Block Kit Messages

The v2 tree is plain `mrkdwn`, which keeps every task ID clickable. Outcome and explicit agent messages use [Block Kit](https://api.slack.com/block-kit) only where it adds structure:
