...
`slack-read` and the worker thread-context helpers also extract text from attachment-only and Block Kit-only messages, so alert threads from tools like Datadog, PagerDuty, and GitHub can be delegated without manually copying the root message text first.
...
If the bot is not yet a member of a **public** channel, `slack-read`, `slack-post`, `slack-reply`, and `slack-start-thread` automatically join it (via the `channels:join` scope) and retry — no manual `/invite` needed. **Private** channels cannot be self-joined: the tools return a clear error asking you to invite the bot with `/invite @<bot-name>` first.
