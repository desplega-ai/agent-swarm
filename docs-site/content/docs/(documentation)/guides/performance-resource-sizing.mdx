...
| Worker class | Recommended pod shape | Concurrency per pod | Scaling rule |
|---|---|---:|---|
| Heavy coding | 1 agent per pod | 1 active task | Scale by adding pods, not by packing multiple coding sessions into one container. |
| Light specialist | 1 agent per pod | 1 active task | Scale horizontally when queue latency matters. |
...
Agent Swarm scales most predictably when each local-runtime worker runs one active task at a time. Increasing per-worker concurrency can be useful for thin relay workers or low-tool tasks, but it multiplies memory peaks and makes local builds contend inside the same container.

| Concurrency profile | Worker count | Parallel tasks per worker | Total active tasks | Recommended host or cluster budget | When to use |
|---|---:|---:|---:|---|---|
...
| Parallel tasks per worker | Multiplies per-container peak memory and tool contention | Keep at `1` for coding, builds, browser automation, and repo-wide tests. |
