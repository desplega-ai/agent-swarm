...
- **Actionable failure reporting**: subprocess startup / parse failures are emitted back to the parent as structured errors, non-TTY runs no longer leak cursor escape sequences into the JSON pipe, and explicit abort sources now survive as descriptive `failureReason` values such as `cancelled` or `graceful_shutdown` instead of collapsing into a generic cancellation.
