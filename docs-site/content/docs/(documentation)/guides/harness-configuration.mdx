...
| `ACP_TARGET_COMMAND` | Custom only | — | Executable to spawn as the custom ACP agent (also accepts the shorter `ACP_COMMAND` alias). Must resolve on `$PATH` or be an absolute path. It receives the worker's full swarm API key as the swarm MCP bearer — point it only at a binary you trust (see [ACP Specifics](#acp-specifics)) |
