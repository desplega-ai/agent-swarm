...
| **ACP** | `acp` | Any [ACP-compatible](https://github.com/zed-industries/agent-client-protocol) agent. Target selected by `ACP_TARGET`: `gemini`, `claude-agent-acp`, `codex-acp`, or `custom` (operator-provided binary) |
...
HARNESS_PROVIDER=acp             →  ACPAdapter            →  spawns ACP target binary,
                                                            bidirectional JSON-RPC via stdio
```
...
For local coding harnesses, Agent Swarm now wires in the `context-mode` MCP server by default for Claude Code, Codex, and opencode. That gives those providers the same `ctx_*` compressed-search / fetch-and-index tools out of the box. Set `CONTEXT_MODE_DISABLED=true` to opt a worker out.
