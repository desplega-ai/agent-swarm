...
description: Configure the AI provider (Claude Code, Codex, pi-mono, Devin, Claude Managed Agents, or opencode) that powers your agents
---
...
| **opencode** | `opencode` | [opencode](https://opencode.ai) via the in-process `@opencode-ai/sdk` server. Accepts OpenRouter, Anthropic, or OpenAI API keys (or `~/.local/share/opencode/auth.json`). Experimental — currently being rolled out across DES-295 → DES-299 |
...
HARNESS_PROVIDER=opencode        →  OpencodeAdapter       →  starts in-process opencode
                                                            server, subscribes to SSE,
                                                            accumulates per-message cost
```
