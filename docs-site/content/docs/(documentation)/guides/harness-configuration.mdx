...
<Tabs items={["opencode", "claude-code-acp", "Gemini CLI", "Custom"]}>
  ...
  <Tab value="claude-code-acp">
    ...
    ACP_TARGET_ENV_KEYS=["CLAUDE_CODE_OAUTH_TOKEN", "ANTHROPIC_API_KEY"]
    ```

    Needs `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY` in the worker env. `ACP_TARGET_ENV_KEYS` explicitly forwards both supported credential options to the bridge process. This is a different code path from the swarm's native `HARNESS_PROVIDER=claude` adapter; prefer `claude` directly unless you specifically need the ACP transport.
  </Tab>
  <Tab value="Gemini CLI">
    ...
    ACP_TARGET_ENV_KEYS=["GEMINI_API_KEY"]
    ```

    Needs `GEMINI_API_KEY` in the worker env. If the installed Gemini CLI version expects another credential, add that key to `ACP_TARGET_ENV_KEYS` instead.
  </Tab>
  <Tab value="Custom">
...
- **No swarm-side credential gate.** The `acp` adapter has no `readyCheck` — the worker reports `ready: true` immediately. The Docker entrypoint does check the target binary at boot: it resolves the command the adapter would spawn (`opencode` for the preset, otherwise `ACP_TARGET_COMMAND` / `ACP_COMMAND`) and exits with `FATAL` if no target is configured or the executable is not on `$PATH`. Outside the Docker image, an unresolvable target still surfaces at session-spawn time (task fails with `ACP target failed during startup: ...`).
