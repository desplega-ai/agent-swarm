...
| **ACP (Agent Client Protocol)** | `acp` | Any [Agent Client Protocol](https://agentclientprotocol.com) agent, spawned as a subprocess and driven over stdio. No swarm-side credentials — the target process owns its own auth |
...
Scope today is deliberately minimal: **one target** (`custom`, driven entirely by env vars below), no named target profiles, no Docker image wiring beyond the generic worker image, no live steering (see the steering table in [Adding a Harness Provider](/docs/guides/harness-providers#live-task-steering) — ACP exposes a single in-flight `session/prompt` and the only interrupt is a full `session/cancel`).

### How it works

The `ACPAdapter` spawns `ACP_TARGET_COMMAND` as a subprocess and speaks ndjson ACP over its stdio using [`@agentclientprotocol/sdk`](https://www.npmjs.com/package/@agentclientprotocol/sdk)'s `ClientSideConnection`. On session start it calls `initialize` then `newSession`, passing the swarm MCP server (`{apiUrl}/mcp`, over HTTP with the standard `Authorization` / `X-Agent-ID` / `X-Source-Task-Id` headers) plus every other MCP server installed on the agent, translated into ACP's `McpServer` shape. Session notifications (`session/update`) are translated into the swarm's normalized `ProviderEvent` stream the same way every other provider's events are.
...
| `ACP_TARGET_COMMAND` | Yes | — | Executable to spawn as the ACP agent (also accepts the shorter `ACP_COMMAND` alias). Must resolve on `$PATH` or be an absolute path |
...
`ACP_TARGET` itself is reserved for future named target profiles; it must be unset or `"custom"` today — any other value fails session creation with `Unsupported ACP target`.
...
<Tabs items={["opencode", "claude-code-acp", "Gemini CLI", "Custom"]}>
  <Tab value="opencode">
    ...
    ACP_TARGET_COMMAND=opencode
    ACP_TARGET_ARGS=["acp"]
    ```

    opencode resolves its own model credentials from `~/.local/share/opencode/auth.json` or the usual provider env vars (`OPENROUTER_API_KEY`, `OPENAI_API_KEY`, etc.) — configure those the same way you would for [`HARNESS_PROVIDER=opencode`](#opencode).
  </Tab>
