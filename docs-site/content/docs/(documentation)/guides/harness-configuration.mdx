...
<Tabs items={["opencode", "claude-code-acp", "Gemini CLI", "Custom"]}>
  ...
  <Tab value="claude-code-acp">
    ...
    Needs `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY` in the worker env. Because this uses the custom ACP target path, list the credential name(s) in `ACP_TARGET_ENV_KEYS` or they will not be passed to the bridge. This is a different code path from the swarm's native `HARNESS_PROVIDER=claude` adapter; prefer `claude` directly unless you specifically need the ACP transport.
  </Tab>
  <Tab value="Gemini CLI">
    ...
    Needs `GEMINI_API_KEY` (or whatever credential the installed Gemini CLI version expects) in the worker env. List the credential name(s) in `ACP_TARGET_ENV_KEYS` so the custom target passes them to the Gemini process.
  </Tab>
  <Tab value="Custom">
