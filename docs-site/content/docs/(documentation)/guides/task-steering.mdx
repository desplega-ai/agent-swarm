...
Every delivered agent-facing envelope starts with a literal
`[steering <uuid>]` marker. After incorporating the message, the assigned agent
passes that UUID to `accept-steer`:
