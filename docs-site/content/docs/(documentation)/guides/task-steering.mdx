...
Steering messages identify the sender by name and kind (human or agent) in the agent prompt and dashboard. Buffered Slack steering retains the human sender's attribution when delivered.
...
Slack thread steering defaults to the lead with queued delivery. Set
`SLACK_THREAD_STEERING=off` to disable it, or choose another mode:
