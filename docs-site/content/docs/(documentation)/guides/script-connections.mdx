...
The runtime validates these top-level location buckets. A misplaced parameter
such as `{ userIds: [...] }` fails with a hint to use the declared location
(for example, `query: { user_ids: [...] }`) instead of silently dropping the
filter. Array-valued query parameters use OpenAPI's default exploded form, so
`["a", "b"]` is sent as `?key=a&key=b`; array-valued headers are appended using
the corresponding HTTP header semantics.
...
- OpenAPI specs may be JSON or YAML (YAML is converted to JSON at ingest and
  stored canonically).
- Array query parameters use OpenAPI's default `form` + `explode: true`
  encoding. Per-operation custom `style` / `explode` settings are not modeled
  yet.
