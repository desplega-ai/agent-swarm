...
| `slack` | `SLACK_BOT_TOKEN` + the selected mode credential (`SLACK_APP_TOKEN` for `socket`, `SLACK_SIGNING_SECRET` for `http`) + `!SLACK_DISABLE` | Not reported yet; configured credentials do not prove live inbound delivery |
