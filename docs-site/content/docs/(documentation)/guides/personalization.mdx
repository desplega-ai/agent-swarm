...
All identity envs are read on every `/status` request. Set them in your `.env`, Docker compose, or via `swarm_config` (then `POST /internal/reload-config`). Unset envs fall back to neutral defaults.
