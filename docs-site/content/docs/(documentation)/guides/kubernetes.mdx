...
The dashboard is served over HTTPS, so the API must be too; browsers block mixed content. The simplest path is [cert-manager](https://cert-manager.io/docs/installation/) with a ClusterIssuer. Name the issuer in the chart and skip the manual TLS Secret:
...
ingress:
  ...
  host: swarm-api.example.com
  certManager:
    clusterIssuer: letsencrypt-prod   # or `issuer:` for a namespaced Issuer
```

The chart adds the `cert-manager.io/cluster-issuer` annotation, generates a `tls` entry for `host` with the Secret `<release>-api-tls`, and derives `PUBLIC_MCP_BASE_URL` as `https://swarm-api.example.com`. Explicit `ingress.tls` entries take precedence when both are set. The same `certManager` block exists under `agentFs.ingress` (Secret `<release>-agent-fs-tls`).

If your ingress controller issues certificates itself (Traefik, Caddy ingress controller) or TLS terminates in a proxy in front of the cluster, leave `certManager` blank, add the controller's annotations, and set `config.publicMcpBaseUrl` explicitly as described below.

### Load balancer health checks

The API returns `401` on `/`, so a load balancer that probes the default path marks every target unhealthy and the ingress never forwards traffic. Probe `/health`, which is unauthenticated. With `ingress.className: alb` the chart sets `alb.ingress.kubernetes.io/healthcheck-path: /health` automatically (also on the agent-fs ingress). For GKE, attach a `BackendConfig` with `healthCheck.requestPath: /health` through `ingress.annotations`. ingress-nginx and Traefik do not probe a path.
...
2. Otherwise, when `ingress.enabled` is true and `ingress.host` is nonempty, derive the URL from that host: `https://` if `ingress.tls` has at least one entry or `ingress.certManager` names an issuer, or `http://` otherwise.
...
The `agentFs.ingress.enabled`, `className`, `annotations`, `host`, `tls`, and `certManager` fields have the same meanings and defaults as the API ingress fields above. Use a separate hostname and matching TLS Secret:
...
Unset or blank `CORS_ALLOWED_ORIGINS` now allows HTTPS subdomains of `agent-swarm.dev` and `agent-swarm.cloud`, plus the documented local development origins. By default, other origins receive neither `Access-Control-Allow-Origin` nor `Access-Control-Allow-Credentials`. The wildcard patterns exclude apex domains.

**Breaking change for custom dashboards:** set `CORS_ALLOWED_ORIGINS=https://dashboard.example.com` on the API, substituting your dashboard origin. A custom list replaces all built-in entries. In Helm, set `config.extraEnv.CORS_ALLOWED_ORIGINS` in your values.yaml as shown below and upgrade the release with that file (`helm upgrade ... -f values.yaml`). See [the complete defaults and migration options](/docs/reference/environment-variables#breaking-change-secure-cors-defaults).
...
For intentional compatibility, the deployment-only `CORS_ALLOW_ANY_ORIGIN=true` environment variable (requires an API restart; unavailable through runtime Configuration) allows arbitrary origins for non-credentialed requests only. Bearer-token clients from unlisted origins must use `credentials: "omit"`. Cookie-authenticated responses, including page JSON and `/@swarm/api/*`, always require an allowlisted origin for credentialed CORS. The API logs a warning once per process when active. The flag never grants `Access-Control-Allow-Credentials` to an unlisted origin; configure trusted origins for page-session access. CORS does not replace authentication or CSRF protection.
