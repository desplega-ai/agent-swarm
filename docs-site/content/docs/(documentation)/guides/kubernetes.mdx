...
Configure a cloud load balancer to probe `/health`, not `/`. The ingress route uses `/` for API traffic, but that endpoint requires authentication and returns `401` to an unauthenticated health check, so the load balancer never marks the backend healthy. For AWS ALB, set `alb.ingress.kubernetes.io/healthcheck-path: /health` and `alb.ingress.kubernetes.io/success-codes: "200"` in the ingress annotations. For GCE, point `cloud.google.com/backend-config` at a BackendConfig whose `healthCheck.requestPath` is `/health`. Apply the same setting to `agentFs.ingress.annotations` when you expose agent-fs through its own load balancer.
