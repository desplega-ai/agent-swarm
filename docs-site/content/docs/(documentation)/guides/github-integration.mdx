...
The worker entrypoint exports this identity as `GIT_AUTHOR_*` and `GIT_COMMITTER_*`, giving it precedence over repository-local Git configuration so later setup steps cannot silently change commit attribution.
