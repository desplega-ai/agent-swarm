...
When GitHub credentials are not also configured, the worker entrypoint exports this GitLab identity as `GIT_AUTHOR_*` and `GIT_COMMITTER_*`, giving it precedence over repository-local Git configuration.
