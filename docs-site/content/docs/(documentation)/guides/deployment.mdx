...
<Callout type="info">
Worker images also install GitHub's official `gh-stack` extension for stacked pull-request workflows. Run `gh stack --help` inside a worker to see the available stack commands; both the slim and full targets include it.
</Callout>
