...
The worker image now also bundles Redis server/client binaries for queue or cache-backed integration tests. Nothing auto-starts unless you set `SWARM_DEP_REDIS_ENABLED=true`; in that case the entrypoint runs [`scripts/init-local-redis.sh`](https://github.com/desplega-ai/agent-swarm/blob/main/scripts/init-local-redis.sh), which starts Redis on `localhost:6379` by default. Override with `LOCAL_REDIS_DATA_DIR` and `LOCAL_REDIS_PORT` as needed.
