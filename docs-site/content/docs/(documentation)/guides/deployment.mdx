...
Per-agent `setupScript` / `/workspace/start-up.*` hooks now run as the non-root `worker` user after the container drops privileges. Use them for project-local setup such as `bun install`, generated config, cache warmup, or repo bootstrap. If you need extra Debian/Ubuntu packages, add them to the worker image or run a root-owned bootstrap step before the entrypoint switches users.
