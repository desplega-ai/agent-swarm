...
AGENT_SWARM_VERSION=                        # Pin a release here. Blank tracks `latest`. See "Pinning the Image Version" below.
...
# Explicit IDs take precedence and are persisted; keep them to retain identity.
...
<Callout type="info">
If you leave `SECRETS_ENCRYPTION_KEY` blank on a brand-new deploy, the API will auto-generate one and write it to the `swarm_api_data` volume at `.encryption-key`. For production, set it explicitly so you control where it lives and can back it up alongside your other secrets. See [Encryption Key](#encryption-key).
</Callout>

### Step 3: Choose Optional Agent ID Overrides
...
All twelve Compose agent ID variables are optional. Leave them blank to generate a UUID on first boot. Each service saves its ID to `/workspace/personal/.agent-id` on its own personal volume and reuses it across restarts. An explicit `.env` value takes precedence and is persisted to that file; keep existing overrides to retain identity. Removing personal volumes discards generated IDs.

If you want explicit overrides, generate one unique UUID per agent and paste the output into the corresponding variables above:

```bash
# Optional: generate `*_AGENT_ID=value` overrides, then paste them into .env.
for name in LEAD_AGENT_ID WORKER_1_AGENT_ID WORKER_2_AGENT_ID CONTENT_WRITER_AGENT_ID CONTENT_REVIEWER_AGENT_ID CONTENT_STRATEGIST_AGENT_ID UX_PRINCIPLES_AGENT_ID DISCOVERABILITY_AGENT_ID RESEARCHER_AGENT_ID REVIEWER_AGENT_ID TESTER_AGENT_ID FORWARD_DEPLOYED_ENGINEER_AGENT_ID; do
  printf '%s=%s\n' "$name" "$(uuidgen)"
done
```
...
The API and every agent service resolve their tag from a single `.env` variable:

```bash title=".env"
AGENT_SWARM_VERSION=1.150.0
```

Pick a version from [releases](https://github.com/desplega-ai/agent-swarm/releases). Confirm every service resolved the same tag:
...
<Callout type="warn">
**Leaving `AGENT_SWARM_VERSION` blank tracks `:latest`, which is not a release.** `:latest` is rebuilt and moved on every commit to `main`; a version tag is published only when the release version changes. The compose example sets `pull_policy: always`, so an unpinned deployment can move to a newer build on any `up -d` or restart — including one you did not intend as an upgrade. Pin anything you depend on.
</Callout>

Changing this value later is an upgrade, not a restart. Migrations are forward-only, so re-pinning to an older tag does **not** roll back a schema change — a database backup taken before the upgrade does. Back up first ([Backing Up the Database](#backing-up-the-database) and [Backing up the key](#backing-up-the-key)), then:
...
Verify with `/health` **and** a completed task, not `/health` alone. On Kubernetes, the chart pins images through its `appVersion`; pass `--version` to `helm upgrade` or it moves to the newest published chart. See [Kubernetes and Helm](/docs/guides/kubernetes).
...
### Step 7: HTTPS with Caddy (optional, needed for the hosted dashboard)
...
Caddy proxies the API domain to `api:3013`. On the agent-fs domain, `/agentfs/*` goes to `minio:9000` for signed image/PDF/video previews and downloads; all other paths go to `agent-fs:7433`. The bucket path and `Host` header are preserved for S3 signature validation, so previews use the existing file domain without another DNS record. Once it runs, stop exposing ports 3013 and 7433 on a public interface (firewall them or remove the `ports` entries). Certificates persist in the `caddy_data` volume. Requires Docker Compose v2.23.1 or newer for the inline Caddyfile.

Signed URLs default to `https://${AGENT_FS_DOMAIN}` when `AGENT_FS_DOMAIN` is set, or `http://localhost:9000` otherwise. A non-empty `AGENT_FS_S3_PUBLIC_ENDPOINT` overrides either default and must be an origin browsers can reach over HTTPS for TLS deployments.

If you already run a reverse proxy (Caddy, nginx, Traefik, Cloudflare Tunnel), skip the profile and set `PUBLIC_MCP_BASE_URL` to the HTTPS API origin that proxy serves. For file previews, set `AGENT_FS_DOMAIN` and configure the same `/agentfs/*` MinIO route without stripping the bucket prefix or rewriting `Host`, or set `AGENT_FS_S3_PUBLIC_ENDPOINT` to a separate browser-reachable HTTPS S3 origin. When self-hosting the dashboard on your own domain, also set `CORS_ALLOWED_ORIGINS` to that origin.
...
─────────────────────────────────────────────────────────────────────
swarm_api_data           → /app/data             → SQLite DB (all swarm state)
...
|--------|-----------|---------|-------------|
| `swarm_api_data` | **Yes** | **Yes** | Contains the SQLite database (`agent-swarm-db.sqlite`) with all tasks, agents, schedules, and configuration. Losing this = losing all swarm state. Mounted at `/app/data` — see [Migrating from the `/app` mount](#migrating-from-the-app-mount). |
...
<h3 id="migrating-from-the-app-mount">Migrating from the `/app` mount</h3>

Installs created before this change mount `swarm_api` at `/app` instead of `swarm_api_data` at `/app/data`.

<Callout type="error">
**Fix this — it silently breaks upgrades.** A Docker named volume copies image content only on *first* population, then shadows that path forever. Mounting `/app` freezes everything the image ships there — `migrations/`, `package.json`, `extensions/`, `scripts-runtime/`, `script-types/`, `typescript-lib/`, `vendored-openapi/` — at install time. The compiled binary lives outside `/app` (`/usr/local/bin/agent-swarm-api`), so **the code upgrades while its migrations do not**. The API then queries columns and tables no migration in its frozen directory created, and every affected write fails with `SQLiteError: table … has no column named …`.
</Callout>

It is undetectable from the outside: `/health` reads the stale `/app/package.json`, so it reports the **old** version and stays green while newer code runs. Pinning the image version does not prevent this — a pinned install breaks the same way on its next upgrade. The two fixes are complementary.

Compare the running container against its own image:
...
<Callout type="warn">
The `swarm_api_data` volume contains your SQLite database — the single source of truth for all swarm state. Back it up regularly.
</Callout>

```bash
# Back up the database, its WAL sidecars, and .page-session-secret
...
docker compose down
docker run --rm -v swarm_api_data:/data -v $(pwd):/backup alpine \
  sh -c 'cp -a /backup/swarm-api-data-backup/. /data/'
docker compose up -d
```
...
docker run --rm -v swarm_api_data:/data -v $(pwd):/backup alpine sh -c '
  cp -a /data/. /backup/swarm-api-data-backup/ &&
  cp /data/.encryption-key /backup/encryption-key-backup 2>/dev/null || true
'
```
...
| `SLACK_MODE` | Slack transport: `socket` (default) or `http`; HTTP remains unavailable until its receiver is installed |
| `SLACK_BOT_TOKEN` | Enable Slack integration; also needs `SLACK_APP_TOKEN` in socket mode or `SLACK_SIGNING_SECRET` in HTTP mode |
...
2. Reuse `entrypoint: *agent-entrypoint` from the example compose file. Leave `WORKER_3_AGENT_ID` blank for a generated, persisted ID, or set it in `.env` to a unique UUID from `uuidgen` as an optional override
...
```bash
# Pin a release. `:latest` moves on every commit to `main` and is not a release.
