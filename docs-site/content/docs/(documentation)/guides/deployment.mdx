...
The worker image also bundles `/usr/local/bin/install-repo-hooks.sh`. If a registered repo is configured with `hooks: { enabled: true }`, the runner calls that helper after clone/refresh so git hooks are installed automatically inside the worker checkout. The helper detects both `.pre-commit-config.yaml` and `prek.toml` before it runs `prek install`.
