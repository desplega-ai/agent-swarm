...
Live deliveries include `[steering <id>] From <sender>: <body>`, where the API resolves the sender's name and kind (user or agent), or `system`. Missing names fall back to the sender ID and kind. The marker remains unchanged for `accept-steer`. The runner uses this envelope for every provider's live delivery; the legacy Codex hook uses the same renderer. Template resolution failures still deliver the bare body. The dashboard displays the same sender label in steering rows, queued previews, and session activity.
...
Memory self-rating hints and server-side citation ratings (`implicit-citation,explicit-self`) default on across harnesses when `MEMORY_RATERS` is unset, including existing deployments. Set `MEMORY_RATERS=` to disable raters; LLM ratings still require an explicit `llm` entry. Config reload preserves the empty opt-out, and deleting an override restores the deployment value (or the runtime default when unset).
