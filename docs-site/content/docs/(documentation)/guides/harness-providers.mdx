...
| [`src/providers/ai-sdk-agent-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/ai-sdk-agent-adapter.ts) | In-process Vercel AI SDK `ToolLoopAgent`; converts MCP tools to AI SDK tools and adds an adapter-local `Skill` tool that loads the current agent's installed skill content on demand | `OPENAI_API_KEY` |

### ai-sdk-agent skill loading

`ai-sdk-agent` exposes a provider-local tool named `Skill` to the model loop. It is not registered in the global MCP server and does not appear in `CORE_TOOLS`, so other harnesses keep using their existing slash-skill resolver path. The tool resolves only the current agent's installed skills, prefers exact normalized name/slug matches, falls back to fuzzy name/description matching, and truncates loaded content with `MAX_SKILL_CHARS`.
