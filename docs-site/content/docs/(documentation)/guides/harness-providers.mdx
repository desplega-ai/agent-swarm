...
| [`src/providers/acp-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/acp-adapter.ts) | Spawns an ACP target over stdio via `@agentclientprotocol/sdk` `ClientSideConnection`; injects the swarm MCP server during `session/new` | Target-dependent — `ACP_TARGET` selects built-in (`gemini-cli`, `claude-agent-acp`, `codex-acp`) or custom (`ACP_COMMAND` + `ACP_ENV_*`) |
...
`HARNESS_PROVIDER=acp` runs an ACP-compatible agent command and treats the swarm
as the ACP client. The default profile is `ACP_TARGET=custom`.

Set `ACP_COMMAND` to the target executable. `ACP_ARGS` accepts either a JSON
string array, such as `["--stdio"]`, or a simple whitespace-split string. Only a
small base environment is forwarded by default; target-specific values use
`ACP_ENV_<NAME>`, which becomes `<NAME>` in the child process.

System prompt handling is target-neutral. If
`ACP_SYSTEM_PROMPT_ARTIFACT_PATH` or `ACP_SYSTEM_PROMPT_PATH` is set, the
adapter writes the composed system prompt to that file before launch. The prompt
is not prepended to the user message unless
`ACP_SYSTEM_PROMPT_FALLBACK=user_message` is explicitly set.

Cost and context reporting stay honest. `ACP_COST_PROVIDER` can point cost
recompute at an existing pricing namespace (`codex`, `gemini`, `opencode`,
etc.); otherwise rows are tagged `acp` and may be `unpriced`. Context snapshots
are emitted only when the target sends ACP `usage_update` values with real
`used` / `size` numbers.
...
All providers are wired together by the factory at [`src/providers/index.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/index.ts):
