...
After a prompt completes, ACP persists a bounded, scrubbed `acp_prompt_response` raw-log diagnostic with the optional usage and `_meta` fields. Reported input/output/cache counters feed cost telemetry; missing usage stays unknown and unresolved pricing stays `unpriced`. See [cost and context computation](/docs/guides/cost-and-context-computation#acp-prompt-usage).
