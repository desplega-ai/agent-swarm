...
| [`src/providers/acp-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/acp-adapter.ts) | `Bun.spawn` of the `ACP_TARGET_COMMAND` process, driven over stdio with `@agentclientprotocol/sdk`'s ndjson `ClientSideConnection` | None swarm-side — the ACP target process owns its own auth |
