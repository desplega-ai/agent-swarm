...
| [`src/providers/acp-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/acp-adapter.ts) | Spawns an Agent Client Protocol target over stdio, injects the swarm MCP server during `session/new`, and translates ACP `session/update` notifications into `ProviderEvent`s | Target-dependent. `ACP_TARGET=custom` delegates auth to the configured `ACP_COMMAND`; pass target env via `ACP_ENV_*` |
