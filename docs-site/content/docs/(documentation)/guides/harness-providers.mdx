...
description: Implement a new harness provider (like claude, pi, codex, or devin) — the full adapter contract, reference implementations, and wiring checklist
---

import { Tab, Tabs } from "fumadocs-ui/components/tabs";
import { Callout } from "fumadocs-ui/components/callout";

This guide documents the harness-provider contract used by agent-swarm workers, the four reference implementations (`claude`, `pi`, `codex`, `devin`), and every hook that must be wired when adding a new provider.
...
**Supported today:** `claude` (Anthropic Claude Code CLI), `pi` (pi-mono, in-process via `@mariozechner/pi-coding-agent`), `codex` (OpenAI Codex SDK), `devin` (Cognition Devin, remote API-driven).
...
|---|---|
| `name` | Short identifier used in logs (`"claude"`, `"pi"`, `"codex"`, `"devin"`). |
...
| [`src/providers/devin-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/devin-adapter.ts) | Remote API sessions via Cognition's v3 REST API; polling loop for status, messages, and structured output | `DEVIN_API_KEY` + `DEVIN_ORG_ID` |
...
export function createProviderAdapter(provider: string): ProviderAdapter {
  switch (provider) {
    ...
    default: throw new Error(`Unknown HARNESS_PROVIDER: "${provider}". Supported: claude, pi, codex, devin`);
  }
}
```
...
All four open the file with `Bun.file(config.logFile).writer()` and append JSONL lines.
...
<Callout type="warn">
  **Secret scrubbing is mandatory at every log egress.** Import `scrubSecrets` from [`src/utils/secret-scrubber.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/utils/secret-scrubber.ts) and wrap every string before writing. All four reference adapters do this; a new adapter must too (see claude L284, L294, L303, L320, L344; pi L170–173; codex L352–355; devin `emit()`).
</Callout>
...
## 7. Skills (how the four providers handle them)
...
<Tabs items={["Native slash-commands", "Prefixed slash-commands", "Inline resolver (Codex)", "Inline resolver (Devin)"]}>
  ...
  <Tab value="Inline resolver (Codex)">
  ...
  <Tab value="Inline resolver (Devin)">
    **Devin.** Devin natively supports skills, but they must be reachable in the repository. To use swarm-managed skills (which aren't committed to the repo), the adapter inlines skill content into the prompt instead. [`src/providers/devin-skill-resolver.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/devin-skill-resolver.ts):

    - Detects `@skills:<name>` on the first line of the prompt via regex.
    - Reads `${DEVIN_SKILLS_DIR ?? plugin/pi-skills}/<name>/SKILL.md`.
    - **Inlines** the SKILL.md content into the prompt before creating the API session.
    - `formatCommand(name)` returns `@skills:${name}`.

    Follows the same inline-resolver pattern as Codex but with a different prefix convention.
  </Tab>
</Tabs>
...
Update the error message (`Supported: claude, pi, codex, devin, foo`) so the unknown-provider error stays accurate.
...
Four patterns are already in the codebase; pick the one that fits your provider:
...
- **Devin**: supports native skills, but they must be reachable in the repository. To use swarm-managed skills, the adapter inlines content via [`src/providers/devin-skill-resolver.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/devin-skill-resolver.ts), which detects `@skills:<name>` on the first line, reads `SKILL.md`, and inlines the content into the prompt. `formatCommand(name)` returns `@skills:${name}`.

Pick the model that matches your runtime; if your provider has no native skill mechanism, follow the Codex/Devin inline-resolver pattern.

The swarm syncs skill files into each provider's skill dir at container boot in [`docker-entrypoint.sh`](https://github.com/desplega-ai/agent-swarm/blob/main/docker-entrypoint.sh) (copies `SKILL.md` into `~/.claude/skills/`, `~/.pi/agent/skills/`, and `~/.codex/skills/`). Devin uses `plugin/pi-skills/` directly via `DEVIN_SKILLS_DIR`. Add an entry for your provider there.
...
Update **every** union-type entry that enumerates providers — the system uses these for validation, DB constraints, and UI rendering:

- [`src/types.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/types.ts) — `ProviderNameSchema` (the Zod enum that is the source of truth: `z.enum(["claude", "codex", "pi", "devin", ...])`) and the derived `ProviderName` type. If your provider has provider-specific metadata (like Devin's `sessionUrl`), add a `FooProviderMeta` type here too.
