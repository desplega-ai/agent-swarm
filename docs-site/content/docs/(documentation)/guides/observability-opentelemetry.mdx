...
```bash title=".env"
OTEL_EXPORTER_OTLP_ENDPOINT=https://ingest.eu2.signoz.cloud/v1/traces
...
Use the region-specific endpoint from your SigNoz Cloud account. For the EU2 region, the traces endpoint is `https://ingest.eu2.signoz.cloud/v1/traces`.
...
```bash
export OTEL_EXPORTER_OTLP_ENDPOINT="https://ingest.eu2.signoz.cloud/v1/traces"
...
| `agentswarm.tool.unclosed` | Session ended before a tool-end event |
...
This is useful when a provider exits without emitting matching tool-end events.
...
- `OTEL_EXPORTER_OTLP_ENDPOINT` ends in `/v1/traces`
