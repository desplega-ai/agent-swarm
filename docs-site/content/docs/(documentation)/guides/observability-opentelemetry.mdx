...
Server-side MCP tool handlers emit `mcp.tool` spans. These nest under the API HTTP request span that triggered them.
...
Server-side `mcp.tool` spans live under `agent-swarm-api` and `worker.mcp.tool` under `agent-swarm`, so this query filters on `service.namespace` to catch both.
