...
- `ctx.swarm` exposes the script SDK through loopback HTTP. Method names follow the SDK (`slack_post`, `task_send`, ...). The `script-query-types` tool or `GET /api/scripts/type-defs` returns `swarm-sdk.d.ts` with the full surface.
