...
A PATCH request can submit unchanged `[REDACTED:...]` placeholders.
...
- `ctx.swarm` exposes the script SDK through loopback HTTP.
...
Its `ctx.swarm` calls use `callOrigin: "extension"`.
