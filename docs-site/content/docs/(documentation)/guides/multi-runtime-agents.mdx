...
Multi-runtime is enabled by default for fresh deployments. Set
`MULTI_RUNTIME_ENABLED=false` explicitly while upgrading older workers.
...
AGENT_SWARM_VERSION=replace-with-a-pinned-release-tag
```

```yaml title="docker-compose.yml"
services:
  api:
    image: ghcr.io/desplega-ai/agent-swarm:${AGENT_SWARM_VERSION:-latest}
    pull_policy: always
    environment:
...
Pin `AGENT_SWARM_VERSION` to a release instead of tracking `latest`, and see
[`upgrade.md`](https://github.com/desplega-ai/agent-swarm/blob/main/skills/agent-swarm/references/upgrade.md)
