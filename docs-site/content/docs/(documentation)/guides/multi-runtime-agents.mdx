...
<Callout type="warn" title="Offered-task recovery is still being tracked">
  Issue [#1190](https://github.com/desplega-ai/agent-swarm/issues/1190) is still
  open. Until it lands, a task already offered to an agent can remain in the
  `offered` state when that agent's last runtime expires. A worker with the
  same agent ID can recover the offer by registering again and polling; a
  permanently dead agent may require manual recovery.
</Callout>
