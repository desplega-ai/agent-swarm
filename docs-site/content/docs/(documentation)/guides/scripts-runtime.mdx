...
KV reads are the notable optional-read exception. Use
`ctx.swarm.kv_getOrNull<T>({ key, namespace? })` when a missing key is expected:
it returns the entry directly, returns `null` on a 404, and throws on other
errors. The original `kv_get` keeps its wrapped response contract for
compatibility. Use `kv_delete` for hard deletes; `kv_del` remains an alias for
existing scripts.
