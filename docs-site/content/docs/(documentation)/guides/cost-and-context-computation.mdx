...
- **acp**: generic Agent Client Protocol wrapper. Custom ACP targets default to `provider='acp'`; those rows are saved and recomputed when matching pricing rows exist, otherwise they are marked `unpriced` and keep the target-reported dollar value. Operators can set `ACP_COST_PROVIDER` to a seeded provider (`codex`, `gemini`, `opencode`, etc.) when the wrapped target's model ids follow that provider's pricing namespace. ACP context snapshots are emitted only for `usage_update` messages with real `used`/`size` values and are tagged `contextFormula='harness-reported'`; the adapter does not synthesize context counts.
...
- **ACP custom targets need an explicit pricing namespace for recompute.** Leaving `ACP_COST_PROVIDER` unset is correct for unknown targets, but most production targets should set it to the underlying model provider so the pricing-table path can find seeded rows.
