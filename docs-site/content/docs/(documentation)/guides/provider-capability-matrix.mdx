...
| OpenAI / Codex | Semantic/hybrid search with `OPENAI_API_KEY` or `EMBEDDING_API_KEY` | Yes; LLM rating when enabled | Yes with `OPENAI_API_KEY` | Native `turn/steer` and queued next-turn input | Yes | Inline resolver, `~/.codex/skills` | Yes | Yes | Model-dependent; Codex also supports `max` on capable models | Yes | Structured API limits and credits cooldown | Yes |
