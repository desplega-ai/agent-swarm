...
| `LEAD_PORT` | No | `3020` | Host port for lead service (used in `docker-compose.example.yml`) |
| `WORKER1_PORT` | No | `3021` | Host port for worker-1 service (used in `docker-compose.example.yml`) |
| `WORKER2_PORT` | No | `3022` | Host port for worker-2 service (used in `docker-compose.example.yml`) |
