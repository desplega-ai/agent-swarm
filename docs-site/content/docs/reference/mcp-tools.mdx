...
Manage recurring task automation. Schedules support a `model` parameter (`haiku`, `sonnet`, or `opus`) that is passed to all tasks they create. See [Scheduled Tasks](/docs/concepts/scheduling) for details.
