...
export function OAuthInlineConnect({
  ...
  // Detection: whenever the (refetched) authorizations include the pending
  // label, auto-select it. Kept separate from the poll timer so the timer isn't
  // reset every refetch (authorizations is a fresh reference each render).
  useEffect(() => {
  ...
  }, [pendingLabel, authorizations, onChange]);

  // Polling: refetch the app list while an authorization is pending; give up on
  // timeout or when the popup closes. Depends only on pendingLabel so the timer
  // survives the refetches it triggers.
  useEffect(() => {
    if (!pendingLabel) return;
    const started = Date.now();
    const timer = setInterval(() => {
      // Only the timeout gives up — a popup that auto-closes on success would
      // otherwise race the detection refetch and show a false failure.
      if (Date.now() - started > POLL_TIMEOUT_MS) {
        clearInterval(timer);
        setPendingLabel((current) => {
          if (current) toast.error("Authorization did not complete. Try again.");
          return null;
        });
        return;
      }
  ...
  async function startAuthorize() {
    ...
    try {
      ...
      const popup = window.open(
        authorizeUrl,
        "oauth-authorize",
        "width=640,height=760,noopener=no",
      );
          ...
          {authorizeMode && !authValue ? (
            <div className="space-y-2 rounded-md border p-3">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {pendingLabel ? "Waiting for popup..." : "Authorize"}
                </Button>
              </div>
              <p className="text-xs text-muted-foreground">
