...
- **Collapse pattern:** user-initiated section toggles (`CollapsibleSection`) animate height + fade via `motion/react` `AnimatePresence` — 200ms open, 150ms close, snappy; under reduced motion the height movement drops and only the fade remains. The header chevron rotates (150ms) instead of swapping glyphs. Never applied to per-row or data-driven surfaces.
