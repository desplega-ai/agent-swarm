...
- **Collapse pattern:** user-initiated section toggles (`CollapsibleSection`, the settings rail) animate height/width + fade via `motion/react` `AnimatePresence` — 200ms open, 150ms close, snappy; under reduced motion the movement drops and only the fade remains. The header chevron rotates (150ms) instead of swapping glyphs. Never applied to per-row or data-driven surfaces (session-log expanders stay unanimated). The global sidebar rail (shadcn `sidebar.tsx`) runs its width/left transitions at 200ms on `ease-swift` — the stock `ease-linear` is banned, like all linear easing outside shimmer liveness loops.
