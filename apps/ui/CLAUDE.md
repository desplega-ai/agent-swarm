...
Standard timings (all `ease-snappy` unless noted): Dialog/AlertDialog 200ms in / 150ms out; Popover/DropdownMenu/Select/Tooltip 150ms in / 100ms out; Sheet/Drawer 400/250 on `ease-swift` (the large-surface exception); `CollapsibleSection` height+fade 200/150 via `AnimatePresence` (reduced motion keeps the fade, drops the height). Keep Radix overlays on the shadcn CSS `animate-in/out` classes — do not rebase them onto motion/react.
