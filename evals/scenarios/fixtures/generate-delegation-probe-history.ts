 ...
 *
 * Emits `delegation-probe-history.sql`: the full base dev-DB dump
 * (`seeded-history.sql` — schema + `_migrations` + minimal seed data) with the
 * single "flux capacitor" task REMOVED and a fresh audit dataset of 20 terminal
 * `agent_tasks` rows appended. The LEAD under test must DELEGATE the audit to
 * two researcher workers (it is forbidden from querying the tasks API itself),
 * and the workers query this seeded history through the swarm API. The merged
 * answer key lives ONLY in these rows (never in any prompt):
 *
 *   completed count                  = 11
 *   failed count                     = 5
 *   cancelled count                  = 4
...
// the ONLY agent_tasks history (the per-status counts must reflect exactly the
// seeded delegation-probe rows). Match the whole INSERT line.
