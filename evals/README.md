...
- `#/runs/:id/cells/:scenario/:config` — per-attempt judgments (incl. agentic-judge tool logs in `raw`), artifacts, and a chat-style transcript viewer parsed from the raw session logs.
...
Judge model precedence: `scenario.judge.model` > run `--judge-model` > `EVAL_JUDGE_MODEL` > `google/gemini-3-flash-preview`.

Scoring per cell: **best@n** (any attempt passed), pass@1, best/avg judge score, total cost (from swarm session-costs), avg duration.
...
- Claude subscription (OAuth) sessions currently produce no priced cost rows — claude cells show no cost; pi/codex do.
