...
Placement: a piece used by more than one flow lives in `components/ui/` (primitive) or `components/shared/` (composed). Flow-only parts stay in the flow folder (for example `components/onboarding/`, `pages/setup/`).
...
| `SuggestionChips` | `components/sessions/suggestion-chips.tsx` | Starter prompts under a composer. One click puts the prompt in the draft. |
...
| `SecretInput` | `components/shared/secret-field.tsx` | A password input with an eye toggle. Every `Input` prop passes through. `autoComplete` defaults to `"off"`. Pass `"new-password"` on forms that store a credential. |
| `SecretField` | `components/shared/secret-field.tsx` | A write-only secret that saves itself. Saved: masked dots + Replace. Never render a stored secret value: the API does not return it. `KEY_RULES` + `checkSecret` hold the per-provider prefix and length rules. |
| `StatusIcon`, `StatusLine`, `SaveIndicator`, `WithIndicator` | `components/shared/status-icon.tsx` | `StatusIcon`: a small icon with its meaning in a tooltip. Tones: `busy` (spinner), `done` (amber check, never green next to amber progress), `saved`, `dirty`, `warning`, `error`, `none` (keeps the slot). One stable root: the tooltip stays mounted, so the live region never remounts. `StatusLine`: icon + a few words. `busy` shimmers, so use it only for real waits. `SaveIndicator`: an autosave phase as a `StatusIcon`. `WithIndicator`: pins the icon inside an input's right edge. |
...
| `InfoTip` | `components/ui/info-tip.tsx` | Use instead of a helper paragraph under a field. One sentence. It is hover-only (`tabIndex={-1}`), so never put required information in it. |
...
Reference: `components/onboarding/use-autosave.ts` and `components/onboarding/use-setup-save.ts`.

- Text fields store about 800 ms after typing stops (`ready: true`).
- Secrets never store while typing: `ready: false`, `readyOnCommit: checkSecret(...).valid`. They store on paste (`usePasteCommit`) or blur, and only after the per-provider prefix and length rule passes.
...
- `useAutosaveScope` + `AutosaveScopeContext` hold the step's Continue while any field saves.
...
- Header, body, and footer share one column constant (`SETUP_COLUMN` in `pages/setup/components/setup-layout.ts`), so their edges line up.
- Top-align content with a fixed top padding. Never center step content vertically: it moves when the content height changes.
- Split views (list + pane) get a fixed height from `sm` up (`sm:h-[34rem]` in `pages/setup/steps/step-integrations.tsx`). Panes scroll inside, so a pane switch does not move the page.
...
- Keyboard shortcuts: one window `keydown` listener. Skip the event when it is `defaultPrevented`, composing, or a repeat. Skip it with any modifier key, with focus in a typing target, with an open dialog, menu, or listbox, on Enter over a focused control, and on Escape while a tooltip is open. Reference: `useSetupShortcuts` in `pages/setup/components/setup-footer.tsx`. Show the key as a `Kbd` on its button.
...
- Module-level state that outlives a route is keyed per connection (`config.apiUrl`), because the dashboard can switch swarms (`setupVisited` in `components/onboarding/onboarding-redirect.tsx`).
