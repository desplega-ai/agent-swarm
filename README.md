...
- **Scheduled & recurring tasks** — cron-based automation for standing work, with schedules that can target agent tasks, workflows, or catalog scripts. Bundled automations declare their required parameters and integrations; if enabled before setup is complete, they remain in `needs_setup` instead of dispatching. [Scheduling →](https://docs.agent-swarm.dev/docs/concepts/scheduling)
...
- **Durable script workflows** — launch background script runs, inspect their journals, and track them from the dashboard when a one-shot `script-run` is too small. User code runs in a credential-free guest while authenticated swarm operations stay behind a capability broker. [Guide →](https://docs.agent-swarm.dev/docs/guides/script-workflow-runs)
