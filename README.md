...
- **Skills & MCP servers** — reusable procedural knowledge, DB-seeded default skills that retain bundled reference/example files, and per-agent MCP servers with scope cascade. [MCP tools →](https://docs.agent-swarm.dev/docs/reference/mcp-tools)
...
- **KV store** — Redis-like namespaced key/value storage with auto-scoped context (Slack thread / PR / Linear issue / page), plus script-safe optional reads (`kv_getOrNull`) and canonical deletes (`kv_delete`). [MCP tools → KV](https://docs.agent-swarm.dev/docs/reference/mcp-tools#kv-tools)
