...
- **Harness & LLM agnostic** — run with Claude Code, OpenAI Codex, pi-mono, Devin, Claude Managed Agents, opencode, or any [ACP-compatible](https://github.com/zed-industries/agent-client-protocol) agent (Gemini CLI, claude-agent-acp, codex-acp, custom). [Harness config →](https://docs.agent-swarm.dev/docs/guides/harness-configuration) · [Add a new provider →](https://docs.agent-swarm.dev/docs/guides/harness-providers)
