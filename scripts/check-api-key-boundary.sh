...
# apps/swarm/src/utils/api-key.ts. Direct access to `process.env.API_KEY` or
...
# call sites.
#
# Forbidden patterns (in apps/swarm/src/, excluding the helper and tests; this includes
# worker-side runtime code under apps/swarm/src/scripts-runtime/):
#   - process.env.API_KEY
#   - process.env.AGENT_SWARM_API_KEY
#
# Tests (`apps/swarm/src/tests/**`) are exempt — they intentionally mutate the raw env
...
ALLOW_FILES=(
  apps/swarm/src/utils/api-key.ts
)

# Scan production source only — exclude tests.
MATCHES=$(grep -rn --include='*.ts' --include='*.tsx' \
  -E 'process\.env\.(AGENT_SWARM_)?API_KEY' \
  apps/swarm/src/ 2>/dev/null \
  | grep -v '^apps/swarm/src/tests/' \
  | grep -v '^apps/swarm/src/utils/api-key\.ts:' \
  || true)

if [ -n "$MATCHES" ]; then
  echo "ERROR: Direct API_KEY env access detected outside apps/swarm/src/utils/api-key.ts."
  echo ""
  echo "All swarm code must use getApiKey() / setApiKey() from apps/swarm/src/utils/api-key.ts"
