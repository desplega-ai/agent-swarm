...
Create the PR with a descriptive title and a body that follows the repository's PR template and conventions (see `/skill:create-pr`, steps 5 and 6). If the repo defines none, include: summary of changes, key changes list, and testing done. Always include `Fixes #<issue-number>` to auto-close the issue on merge.
