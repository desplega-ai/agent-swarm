...
```json
{"name":"Linear drain loop","description":"Drain ready child issues from a parent issue.","triggerSchema":{"type":"object","required":["parentIssueKey","repoUrl"],"properties":{"parentIssueKey":{"type":"string"},"repoUrl":{"type":"string"}}},"nodes":[{"id":"triage","type":"agent-task","config":{"template":"Review child issues under {{trigger.parentIssueKey}} in project {{LINEAR_PROJECT_ID}}. Identify ready, blocked, duplicate, and needs-human-decision items."},"next":["dispatch"]},{"id":"dispatch","type":"agent-task","inputs":{"triage":"triage"},"config":{"template":"For ready items only, create or assign implementation tasks against {{trigger.repoUrl}}. Leave a tracker comment summarizing what was dispatched."}}]}
```
...
1. **Triage node (lead):** Reads all child issues under `parentIssueKey` in the configured `LINEAR_PROJECT_ID`. Classifies each as: ready (has acceptance criteria, unblocked), blocked (waiting on another issue or external dependency), duplicate (same scope as an existing issue), or needs-human-decision (ambiguous scope or missing spec).
...
- **`LINEAR_PROJECT_ID`** is required install-time configuration so the triage agent cannot search the wrong project.
